...
#include <utils/Log.h>
#undef LOG_TAG
#define LOG_TAG "ChagallOMXPlugin"
...
ChagallOMXPlugin::ChagallOMXPlugin()
    : mLibHandle(dlopen("libchagallOmxCore.so", RTLD_NOW)),
        mInit(NULL),
        mDeinit(NULL),
...
OMX_ERRORTYPE ChagallOMXPlugin::makeComponentInstance(
        const char *name,
...
OMX_ERRORTYPE ChagallOMXPlugin::destroyComponentInstance(
        OMX_COMPONENTTYPE *component) {
    if (mLibHandle == NULL) {
...
OMX_ERRORTYPE ChagallOMXPlugin::enumerateComponents(
        OMX_STRING name,
        size_t size,
        OMX_U32 index) {
    if (mLibHandle == NULL) {
...
OMX_ERRORTYPE ChagallOMXPlugin::getRolesOfComponent(
        const char *name,
