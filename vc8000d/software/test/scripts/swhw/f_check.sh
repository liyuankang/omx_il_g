...
export LD_LIBRARY_PATH=/export/work/sw_tools/libav/i386/usr/local/lib/
...
isNegativeCase8190()
{
    ...
    if [ "$MEM_SIZE_MB" -lt "383" ]&&[ "$case_number" == 10301 ]
    then
        return 1
    fi
    ...
    if [ "$DEC_MULTICORE_DECODE" == "ENABLED" ] && [ "$case_number" -eq 7233 ]
    then
        return 1
    fi
...
prepareH264Case()
{
    ...
    if [ "$TB_H264_TOOLS_NOT_SUPPORTED_CHECK" != "DISABLED" ]
    then
        tools_not_supported=`echo "$TB_H264_TOOLS_NOT_SUPPORTED_CHECK" | tr " " "_"`
        ...
        do
            ...
            then
                ...
                not_supported_tool_in_strm_tmp=`grep -i "$tool_not_supported" ${TEST_DATA_HOME_CHECK}/case_${case_number}/decoding_tools.trc | awk '{print $1}'`
    ...
    local reference_decoder="$H264_DECODER_REF"
    local input_stream="${TEST_DATA_HOME_CHECK}/case_${case_number}/${H264_STREAM}"
    # use multicore reference to get bit axact result for the case
    if [[ "$case_number" -eq "6226" || "$case_number" -eq "6277" || "$case_number" -eq "6228" || "$case_number" -eq "6229" || "$case_number" -eq "6304" || "$case_number" -eq "6305" || "$case_number" -eq "6364" || "$case_number" -eq "6365" || "$case_number" -eq "7233" ]]&&[ "$DEC_MULTICORE_DECODE" == "ENABLED" ]
    then
        reference_decoder="$H264_DECODER_REF_MC"
        if [ -e "${TEST_DATA_HOME_CHECK}/case_${case_number}/${H264_STREAM_MC}" ]
        then
            input_stream="${TEST_DATA_HOME_CHECK}/case_${case_number}/${H264_STREAM_MC}"
        fi
    fi

    runModel "$case_number" "$reference_decoder" "-N$pics $input_stream"
}
...
prepareVP8Case()
{
    ...
    local reference_decoder="$VP8_DECODER_REF"
    # use multicore reference to get bit axact result for the case
    if [ "$case_number" -eq "3538" ]&&[ "$DEC_MULTICORE_DECODE" == "ENABLED" ]
    then
        reference_decoder="$VP8_DECODER_REF_MC"
    fi

    runModel "$case_number" "$reference_decoder" "-N$pics ${stream}"
}
...
preparePpCase()
{
    ...
    then
        ...
        runModel "$case_number" "$PP_VP8_PIPELINE_REF" "-N$TB_MAX_NUM_PICS ${stream} ${TEST_DATA_HOME_CHECK}/case_${case_number}/${PP_CFG}"
    fi
    ...
    then
        ...
        runModel "$case_number" "$PP_AVS_PIPELINE_REF" "-N$TB_MAX_NUM_PICS ${stream} ${TEST_DATA_HOME_CHECK}/case_${case_number}/${PP_CFG}"
    fi
}
...
checkPositiveCase()
{
    ...
    if [ "$decoder" == "h264" ] && [ "$TB_H264_TOOLS_NOT_SUPPORTED_CHECK" != "DISABLED" ]
    then
        tools_not_supported=`echo "$TB_H264_TOOLS_NOT_SUPPORTED_CHECK" | tr " " "_"`
        tools_not_supported=`echo "$tools_not_supported" | tr "," " "`
        for tool_not_supported in $tools_not_supported
...
checkNegativeCase8190()
{
    ...
    then
        ...
        if [ -z "$error_returned" ]
        then
             error_returned=`grep "TB: H264MCDecDecode returned: H264DEC_STRM_PROCESSED" case_${case_number}${TB_LOG_FILE_SUFFIX}`
        fi
        if [ -z "$error_returned" ]
        then
            comment="H264DEC_STRM_PROCESSED return value missing"
    ...
    then
        ...
        if [ -z "$error_returned" ]
        then
             error_returned=`grep "TB: H264MCDecDecode returned: H264DEC_STRM_ERROR" case_${case_number}${TB_LOG_FILE_SUFFIX}`
        fi
        if [ -z "$error_returned" ]
        then
            comment="H264DEC_STRM_ERROR return value missing"
