...
if [ "$TARGET" == "pclinux" ]
then
    g_compile_params="CORES=1 SYSROOT=/export/work/sw_tools/libav/i386"
else
    g_compile_params="SYSROOT=/export/work/sw_tools/libav/arm"
fi
#-------------------------------------------------------------------------------
#-                                                                            --
...
compileDecoderTestBench()
{
    ...
    then
        ...
        then
            ...
            then
                make ENABLE_2ND_CHROMA_FLAG=y SET_EMPTY_PICTURE_DATA=128 $g_compile_params $TARGET >> $BUILD_LOG 2>&1 
            else
                make SET_EMPTY_PICTURE_DATA=128 $g_compile_params $TARGET >> $BUILD_LOG 2>&1 
            fi
        else
            if [[ "$PRODUCT" == "9170" || "$PRODUCT" == "9190"  || "$PRODUCT" == "g1" ]] && [ "$tb" == "mpeg4" ]
            then
                make CUSTOM_FMT_SUPPORT=y $TARGET $g_compile_params >> $BUILD_LOG 2>&1
            ...
            then
                make SET_EMPTY_PICTURE_DATA=128 $TARGET $g_compile_params >> $BUILD_LOG 2>&1
            #elif [ "$PRODUCT" == "g1" ] && [ "$tb" == "vp8" ]
            #then
            #    make SET_EMPTY_PICTURE_DATA=128 $TARGET >> $BUILD_LOG 2>&1
            else
                make $TARGET $g_compile_params >> $BUILD_LOG 2>&1 
            fi
        fi
        if [ ! -f "$bin" ]
        then
...
if [ "$PRODUCT" == "8170" ]
then
    dec_bin="hx170dec_${TARGET}"
    if [ "$TARGET" == "versatile" ]
    then
        dec_bin="hx170dec-arm"
    fi
    compileDecoderTestBench "h264" "$dec_bin"
...
then
    ...
    compileDecoderTestBench "h264high" "$dec_bin"
fi
