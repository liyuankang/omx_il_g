...
setConfigurations()
{
    ...
    then
        ...
        then
            ...
            then
                ...
                appendConfigurations "integration_relset integration_default integration_wholestrm"
        fi
        if [ ! -z "$h264bp" ]
        then
        ...
        then
            ...
            then
                ...
                then
                    ...
                    appendConfigurations "integration_memext_stride"
                    if [ "$FOURK_CASES_ONLY" == "ENABLED" ]
                    then
    ...
    #then
    #    pp_found=""
    #    echo "Excluding Post-Processor test cases due to multicore"
    #fi
    if [ ! -z "$pp_found" ]
    then
    ...
    fourk_excluded=""
    if [[ "$MEM_SIZE_MB" -lt "383" &&  ! -z "$pp_found" ]]||[[ "$MEM_SIZE_MB" -lt "192" && -z "$pp_found" ]]
    then
        fourk_excluded="_4k"
    fi
...
#sed "s,HLINA_START[ \t]*:=.*,HLINA_START := $MEM_BASE_ADDRESS," ../../../linux/memalloc/Makefile > tmp
...
sed "s,KDIR[ \t]*:=.*,KDIR := $KDIR," ../../../linux/ldriver/Makefile > tmp
mv tmp ../../../linux/ldriver/Makefile
...
then
    ...
    sed "s/#define[ \t]*DEC_X170_USING_IRQ[ \t].*/#define DEC_X170_USING_IRQ 1/" ../../../source/config/deccfg.h > tmp
    mv tmp ../../../source/config/deccfg.h
    sed "s/#define[ \t]*PP_X170_USING_IRQ[ \t].*/#define PP_X170_USING_IRQ 1/" ../../../source/config/ppcfg.h > tmp
    mv tmp ../../../source/config/ppcfg.h
...
then
    ...
    mv tmp ../../../linux/dwl/Makefile
    sed "s/#define[ \t]*DEC_X170_USING_IRQ[ \t].*/#define DEC_X170_USING_IRQ 0/" ../../../source/config/deccfg.h > tmp
    mv tmp ../../../source/config/deccfg.h
    sed "s/#define[ \t]*PP_X170_USING_IRQ[ \t].*/#define PP_X170_USING_IRQ 0/" ../../../source/config/ppcfg.h > tmp
    mv tmp ../../../source/config/ppcfg.h
fi
...
then
    # md5sum is enabled from cmdline
    #if [ "$PRODUCT" == "8170" ]
    #then
...
then
    ...
    then
	    ...
	    then
            ...
            #sw_major=`echo "$SWTAG" |awk -F_ '{print $2}'`
            #sw_minor=`echo "$SWTAG" |awk -F_ '{print $3}'`
...
#fi
alloc_size="alloc_size=${MEM_SIZE_MB}"
alloc_base="alloc_base=${MEM_BASE_ADDRESS}"
hw_base_addr="base_port=${HW_BASE_ADDRESS}"
if [ "$MULTICORE_DECODE" == "ENABLED" ]
then
      hw_base_addr="multicorebase=${HW_BASE_ADDRESS},${HW_BASE_ADDRESS_2}"
fi
hw_irq="irq=${IRQ_LINE}"
...
LDRIVER_PATH="${PWD}/../../../linux/ldriver"
...
then
    ...
    echo "cd ../../../linux/memalloc && ./memalloc_load.sh ${alloc_base} ${alloc_size}" >>  loadmodules.sh
    echo "/sbin/rmmod hx170dec" >>  loadmodules.sh
    echo "cd ../ldriver/ && ./driver_load.sh ${hw_base_addr} ${hw_irq}" >>  loadmodules.sh
    ...
    wdir="$PWD"
    echo "\"cd ${wdir}/../../../linux/memalloc && ./memalloc_load.sh  ${alloc_base} ${alloc_size}\""  >>  loadmodules.sh
    echo -n "rsh root@${TB_REMOTE_TEST_EXECUTION_IP} "  >>  loadmodules.sh
    echo "\"cd ${wdir}/../../../linux/ldriver/ && ./driver_load.sh ${hw_base_addr} ${hw_irq}\""  >>  loadmodules.sh
fi
...
do
    ...
    if [[ "$PRODUCT" == "8190" && "$hw_major" -eq "1" && "$hw_minor" -ge "89" ]] || [[ "$PRODUCT" == "8190" && "$hw_major" -eq "0" ]]
    then
        sed "s/export DEC_REFERENCE_BUFFER_INTERLACED=.*/export DEC_REFERENCE_BUFFER_INTERLACED=\"DISABLED\"/" tmpcfg > tmp
        mv tmp tmpcfg
    else
        sed "s/export DEC_REFERENCE_BUFFER_INTERLACED=.*/export DEC_REFERENCE_BUFFER_INTERLACED=\"ENABLED\"/" tmpcfg > tmp
        mv tmp tmpcfg
    fi

    # write DEC_REFERENCE_BUFFER_TOP_BOT_SUM
    if [[ "$PRODUCT" == "8190" && "$hw_major" -eq "2" && "$hw_minor" -ge "83" ]] || [[ "$PRODUCT" == "8190" && "$hw_major" -le "1" ]]
    then
        sed "s/export DEC_REFERENCE_BUFFER_TOP_BOT_SUM=.*/export DEC_REFERENCE_BUFFER_TOP_BOT_SUM=\"DISABLED\"/" tmpcfg > tmp
        mv tmp tmpcfg
    else
        sed "s/export DEC_REFERENCE_BUFFER_TOP_BOT_SUM=.*/export DEC_REFERENCE_BUFFER_TOP_BOT_SUM=\"ENABLED\"/" tmpcfg > tmp
        mv tmp tmpcfg
    fi
    ...
    # write DEC_MULTICORE_DECODE
    sed "s/export DEC_MULTICORE_DECODE=.*/export DEC_MULTICORE_DECODE=\"${MULTICORE_DECODE}\"/" tmpcfg > tmp
    mv tmp tmpcfg
    ...
    echo '$1' >>  checkall.sh
    echo "./checktest.sh --failed force_check" >>  checkall.sh
    echo "cd .." >>  checkall.sh
    echo "" >>  checkall.sh
done
