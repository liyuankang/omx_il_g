...
printUsage()
{
    ...
    echo "force_check         Rerun the model when case_nbr == --failed, otherwise existing data is used"
...
#    test_cases="$1"
    ...
    if [ "$force_check" -eq 1 ] && [ "$case_number" != "--failed" ]
    then
        echo "force_check requires --failed"
        printUsage
        exit 1
    fi
...
checkCases "$test_cases" "$generate_csv_report" "$use_failed" "$force_check"
if [ "$case_number" != "--failed" ]
then
