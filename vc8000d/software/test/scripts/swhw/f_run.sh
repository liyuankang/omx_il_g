...
randomizeCfg()
{
    ...
    # randomize service merge disable: enabled == 1, disabled == 0
    random_value=$RANDOM; let "random_value %= 2"
    if [ "$random_value" == 1 ]
    then
        DEC_SERVICE_MERGE_DISABLE="ENABLED"
    else
        DEC_SERVICE_MERGE_DISABLE="DISABLED"
    fi
    ...
    random_value=$RANDOM; let "random_value %= 2"
    if [ "$random_value" == 1 ]&&[ "$DEC_MULTICORE_DECODE" == "DISABLED" ]
    then
...
writeConfiguration()
{
    ...
    if [ "$DEC_SERVICE_MERGE_DISABLE" == "ENABLED" ]
    then
        echo "    ServiceMergeDisable = 1;"  >> tb.cfg
    else
        echo "    ServiceMergeDisable = 0;"  >> tb.cfg
    fi
    ...
    #if [ "$case_number" == 10350 ]||[ "$case_number" == 6567 ]
    #then
    #    echo "    DwlRefFrmBufferSize = 999999999;"  >> tb.cfg
    #fi
    echo "}"  >> tb.cfg
...
runH264Case()
{
    ...
    case_dir="${TEST_DATA_HOME_RUN}/case_${case_number}"
    local instream=""
    local use_mp4_stream=0

    # check if there is an mp4 stream
    if [ "$DEC_MULTICORE_DECODE" == "ENABLED" ]
    then
        #if [ -e ${case_dir}/decoding_tools.trc ]
        #then
        #    local multislice=`grep Multiple ${case_dir}/decoding_tools.trc | awk '{ print $1 }'`
        #    local fmo=`grep FMO ${case_dir}/decoding_tools.trc | awk '{ print $1 }'`
	#    local aso=`grep ASO ${case_dir}/decoding_tools.trc | awk '{ print $1 }'`
	#    local error=`grep Error ${case_dir}/decoding_tools.trc | awk '{ print $1 }'`
        #    if [ "$multislice" -eq 1 ] && [ "$fmo" -eq 0 ] &&  [ "$aso" -eq 0 ] &&  [ "$error" -eq 0 ]
        #    then
        #        use_mp4_stream=1
        #    fi
        #fi
        if [ -e "${case_dir}/${H264_STREAM_MC}" ]
        then
            use_mp4_stream=1
        fi
    fi
    if [ "$use_mp4_stream" -eq 1 ]
    then
        if [ ! -e "${case_dir}/${H264_STREAM_MC}" ]
        then
            echo "NOK (${case_dir}/${H264_STREAM_MC} missing)"
            return 1
        else
            instream="${TEST_DATA_HOME_ROOT_REXEC}/case_${case_number}/${H264_STREAM_MC}"
        fi
    else
    ...
    # run the case
    tb_params=""
    if [ -e "$H264_DECODER" ]&&[ -e "$H264_DECODER_MC" ]
    then
        if [ "$TB_MD5SUM" == "ENABLED" ]
        then
            tb_params="--md5 ${tb_params}"
        fi
        if [ "$DEC_TILED_MODE" == "ENABLED" ]
        then
        ...
        run_case=1
        if [ "$TB_H264_TOOLS_NOT_SUPPORTED_RUN" != "DISABLED" ]
        then
            tools_not_supported=`echo "$TB_H264_TOOLS_NOT_SUPPORTED_RUN" | tr " " "_"`
            ...
            do
                ...
                then
                    ...
                    not_supported_tool_in_strm_tmp=`grep -i "$tool_not_supported" ${TEST_DATA_HOME_RUN}/case_${case_number}/decoding_tools.trc | awk '{print $1}'`
                    if [ "$not_supported_tool_in_strm_tmp" -eq 1 ]
                    then
                        run_case=0
                        echo "NOT_SUPPORTED"
        ...
        then
            ...
            # run the case on remote host using rexec
                ...
                rsh "root@${TB_REMOTE_TEST_EXECUTION_IP}" "cd ${wdir} && ./rexecruntest.sh \
                        "$wdir" "$decoder" '"${tb_params}"' "case_${case_number}${TB_LOG_FILE_SUFFIX}" \
                        "$decoder_out" "$case_number" "$execution_time""
            fi
        fi
        
    else
        echo "NOK (${H264_DECODER} or ${H264_DECODER_MC} missing)"
        return 1
    fi
    return 0
}
...
runVP8Case()
{
    ...
    local vp8_decoder="$VP8_DECODER"
    getCategory "$case_number"
    local webp_category=`echo $category | grep webp`
    if [ "$DEC_MULTICORE_DECODE" == "ENABLED" ]&&[ -z "$webp_category" ]
    then
        vp8_decoder="$VP8_DECODER_MC"
    fi
    
    # run the case
    if [ -e "$VP8_DECODER" ]&&[ -e "$VP8_DECODER_MC" ]
    then
        # run the case locally
        if [ "$TB_REMOTE_TEST_EXECUTION_IP" == "LOCAL_HOST" ]
        then
        ...
        echo "NOK (${VP8_DECODER} or ${VP8_DECODER_MC} missing)"
        return 1
    fi
    return 0
}
...
runPpCase()
{
    ...
    then
        ...
        then
            ...
            if [ "$VP8_EXT_MEM_ALLOC_ORDER" == "ENABLED" ]
            then
                test_bench_args="-Xa ${test_bench_args}"
            fi
...
runCase()
{
    ...
    then
        ...
        echo "0" > "case_${case_number}.time"
        chmod 666 "case_${case_number}.time"
        echo "" > case_${case_number}_tb_params
        chmod 666 case_${case_number}_tb_params
        return 1
    fi 
    
    # if enabled, randomize all possible parameters
    ...
    then
        ...
        if [ "$DEC_MULTICORE_DECODE" == "DISABLED" ]
        then
            if [[ "$PRODUCT" == "g1" && "$g_hw_major" -eq "1" && "$g_hw_minor" -ge "88" ]] || [[ "$PRODUCT" == "g1" && "$g_hw_major" -ge 2 ]]&&[ "$DEC_MULTICORE_DECODE" == "DISABLED" ]
            then
                random_value=$RANDOM; let "random_value %= 2"
                if [ "$random_value" == 1 ]
                then
    ...
    then
        ...
        then
            ...
            do
                ...
                elif [ "$configuration_item" == "DEC_SERVICE_MERGE_DISABLE" ]
                then
                    export DEC_SERVICE_MERGE_DISABLE="$value"
...
DEC_SERVICE_MERGE_DISABLE=${DEC_SERVICE_MERGE_DISABLE} \
