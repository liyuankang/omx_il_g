...
static int vcmd_pcie_init(void)
{
    ...
    if (!request_mem_region(vcmd_buf_mem_pool.busAddress, vcmd_buf_mem_pool.size,
                              "vc8000_vcmd_driver")) {
        printk(KERN_INFO "Init: failed to request hw region vcmd_buf_mem_pool.\n");
        return -1;
    }
    ...
    if (!request_mem_region(vcmd_status_buf_mem_pool.busAddress, vcmd_status_buf_mem_pool.size,
                          "vc8000_vcmd_driver")) {
        printk(KERN_INFO "Init: failed to request hw region vcmd_status_buf_mem_pool.\n");
        return -1;
    }
    ...
    if (!request_mem_region(vcmd_registers_mem_pool.busAddress, vcmd_registers_mem_pool.size,
                          "vc8000_vcmd_driver")) {
        printk(KERN_INFO "Init: failed to request hw region vcmd_registers_mem_pool.\n");
        return -1;
    }
...
static int vcmd_reserve_IO(void)
{
    ...
    for (i=0;i<total_vcmd_core_num;i++)
    {
      ...
      {
          printk(KERN_INFO "hantrovcmd: failed to reserve HW regs for vcmd %d\n", i);
          printk(KERN_INFO "hantrovcmd: vcmd_base_addr = 0x%08lx, iosize = %d\n",
