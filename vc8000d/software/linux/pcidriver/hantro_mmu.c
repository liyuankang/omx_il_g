...
static enum MMUStatus SetupDynamicSpace(void) {
  ...
  if(pcie) {
    ...
    pointer = dma_alloc_coherent(&platformdev->dev, num_entries*MMU_STLB_4K_SIZE,
		                 &dma_handle, GFP_KERNEL | GFP_DMA);
    g_mmu->stlb_virtual = pointer;
    MMUDEBUG(" *****g_mmu->stlb_virtual = 0x%p**%d\n", pointer, num_entries*MMU_STLB_4K_SIZE);
    g_mmu->stlb_physical = address = (unsigned long long)dma_handle;
    MMUDEBUG(" *****stlb_physical = 0x%08x\n", (unsigned int)g_mmu->stlb_physical);
...
enum MMUStatus MMUInit(volatile unsigned char *hwregs) {
  ...
  if(!hwregs || (ioread32((void*)(hwregs + MMU_REG_OFFSET))>>16) != 0x4D4D)
    return MMU_STATUS_NOT_FOUND;
  ...
  if(platformdev == NULL) {
    printk(KERN_ERR "hantrodec create platform device fail\n");
    status = MMU_STATUS_FALSE;
    goto onerror;
  } else {
    printk(KERN_INFO "Create platform device success\n");
  }

  result = platform_driver_register(&hantro_drm_platform_driver);
  printk(KERN_NOTICE "Platform driver status is %d\n", result);
...
enum MMUStatus MMUCleanup(volatile unsigned char *hwregs) {
  ...
  printk(KERN_INFO "Unregister platform device.\n");
...
static enum MMUStatus MMUEnable(volatile unsigned char *hwregs) {
  ...
  if(pcie) {
    ...
    g_mmu->mtlb_size = MMU_MTLB_SIZE;
    g_mmu->mtlb_virtual = dma_alloc_coherent(&platformdev->dev, g_mmu->mtlb_size,
		                             &dma_handle, GFP_KERNEL | GFP_DMA);
  ...
  *((unsigned long long *)g_mmu->page_table_array) =
                             ...
                             | (0 << 0);
  /* a workaround for Fifo overflow */
