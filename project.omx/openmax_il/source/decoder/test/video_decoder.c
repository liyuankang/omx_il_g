...
#define FRAMECNT 0xFFFFFFFF // Frame number when state transition sequence is started
#define TB_FLUSH
//#define TB_SEEK
//#define TB_CHANGE_CHANNEL
// for testing on PC only. does not work on HW
#define CLIENT_ALLOC
#define USE_VOUT_TH
//#define VOUT_WEIGHT_USEC (1001000/30)
//#define VOUT_WEIGHT_USEC (33000)
#define VOUT_WEIGHT_USEC 90000

#if defined(USE_VOUT_TH)
#include "queue.h"
#endif // defined(USE_VOUT_TH)

//#define LEADBUF_AT_ONCE
//#define STOP_BY_INBUFCOUNT
...
u8 HevcGetNalType(u8 *pByteStream, u32 len);
u8 H264GetNalType(u8 *pByteStream, u32 len);
u8 Mpeg2GetStartCode(u8 *pByteStream, u32 len);
u8 Vp9IsKeyFrame(u8 *pByteStream, u32 len);
u8 AvsGetStartCode(u8 *pByteStream, u32 len);
u8 Vp8IsKeyFrame(u8 *pByteStream, u32 len);
u8 Vp6IsKeyFrame(u8 *pByteStream, u32 len);
u8 Mpeg4GetStartCode(u8 *pByteStream, u32 len);
u8 SorensonGetStartCode(u8 *pByteStream, u32 len);
u8 Divx3GetStartCode(u8 *pByteStream, u32 len);
u8 VC1GetStartCode(u8 *pByteStream, u32 len);
u8 RvGetStartCode(u8 *pByteStream, u32 len);

#if 1
#define LOG_INBUF(name, buff)
#define LOG_OUTBUF(name, buff)
#else
#define LOG_INBUF(name, buff) \
{ \
	struct timeval tv; \
...
{ \
	...
	struct timezone tz; \
	gettimeofday(&tv, &tz); \
	printf("OUTBUF:[%03d:%06d]%s %p %" PRIx64 "\n", \
		(int)(tv.tv_sec) % 1000, (int)tv.tv_usec, \
		name, \
		(buff), \
		(uint64_t)(((ALLOC_PRIVATE*)((buff)->pInputPortPrivate))->nBusAddress)); \
}
#endif

#if defined(USE_VOUT_TH)

volatile OMX_BOOL   run_voutth;
pthread_t vout_thread = (pthread_t)0;

QUEUE_CLASS outqueue;
...
OMX_U32 abort_flag = 0;
OMX_U32 seek_flag = 0;
OMX_U32 empty_flag = 1;
OMX_U32 coding_type;

void vout_thread_main(void * data)
{
	FILLDONEARGS args;

    while(run_voutth == OMX_TRUE)
    {
        if(!abort_flag) {
            int ret = queue_pop(&outqueue, (void*)(&args));
            ...
            usleep(VOUT_WEIGHT_USEC); // weight of vout
            if(abort_flag)
            {
               queue_push(&outqueue, &args);
               empty_flag = 0;
               continue;
            }
            ...
            {
#ifdef CLIENT_ALLOC
                if (args.buffer->pBuffer &&
                   (coding_type != OMX_VIDEO_CodingVP9 ||
                   (coding_type == OMX_VIDEO_CodingVP9 && ctsTest))) {
                    free(args.buffer->pBuffer);
                 }
                else
                {
                    OSAL_MutexLock(realloc_mutex);
                    noReallocBufList.address[noReallocBufList.count] = args.buffer->pBuffer;
                    noReallocBufList.size[noReallocBufList.count] = args.buffer->nAllocLen;
                    noReallocBufList.count++;
                    OSAL_MutexUnlock(realloc_mutex);
                }

#endif
                if (args.buffer)
                    (args.comp)->FreeBuffer(args.comp, 1, args.buffer);
                continue;
            }

            (args.comp)->FillThisBuffer((args.handle), (args.buffer));
            if(queue_size(&outqueue) == 0)
                empty_flag = 1;

            logname = "fillthis";
        }

        LOG_OUTBUF(logname, args.buffer);
...
#if 1
            OUTPUT_BUFFER_PRIVATE *output_data = (OUTPUT_BUFFER_PRIVATE*) buffer->pOutputPortPrivate;
            if (buffer->pOutputPortPrivate != NULL && output_data->nLumaSize) // Get address from output port private pointer
            {
...
/* Stop sending frames to the decoder after FRAMECNT */

    printf("vid_props->framecount %d\n", vid_props->framecount);
//    if (vid_props->framecount <= FRAMECNT)
    {
        buffer->nFilledLen = 0;
        buffer->nOffset    = 0;
        buffer->nTimeStamp = 0;
...
#if defined(USE_VOUT_TH)
        FILLDONEARGS args;
        args.comp   = (OMX_COMPONENTTYPE*)comp;
        args.handle = comp;
        args.buffer = buffer;
        ...
        case OMX_EventPortSettingsChanged:
            {
                ...
                /* make sure no output buffers sent to decoder any more */
                //while(fill_buffer == 0)
                //    usleep(VOUT_WEIGHT_USEC);
                ...
                switch (cmd)
                {
                    case OMX_CommandPortDisable:
                    {
                        ...
                        if (reconfigPending)
                        {
...
#if 0
                            if (port.format.image.nFrameWidth > 4096 || port.format.image.nFrameHeight > 4096)
                            {
                                OMX_U32 scale_ratio = 2.2;
                                port.format.image.nStride = 0;
                                port.format.image.nSliceHeight = 0;
                                port.format.image.nFrameWidth = port.format.image.nFrameWidth / scale_ratio;
                                port.format.image.nFrameHeight =  port.format.image.nFrameHeight / scale_ratio;
                            }
                            ...
                            ((OMX_COMPONENTTYPE*)comp)->GetParameter(comp, OMX_IndexParamPortDefinition, &port);
                            printf ("valid width = %d, valid height = %d, stride = %d, sliceHeight = %d\n",
...
OMX_ERRORTYPE setup_component(OMX_COMPONENTTYPE* comp, const TESTARGS* args)
{
    ...
    if (args->coding_type == OMX_VIDEO_CodingHEVC && dst.nBufferCountActual == 1)
    {
        OMX_VIDEO_PARAM_CONFIGTYPE conf;
        INIT_OMX_TYPE(conf);
        conf.nPortIndex = 1; // output port
        comp->GetParameter(comp, OMX_IndexParamVideoConfig, &conf);
        //conf.bStillPicture = OMX_TRUE;
        err = comp->SetParameter(comp, OMX_IndexParamVideoConfig, &conf);
        if (err != OMX_ErrorNone)
            goto FAIL;
    }
    ...
    if (args->pixFormat != 1)
    {
        ...
        conf.nPortIndex = 1; // output port
        comp->GetParameter(comp, OMX_IndexParamVideoConfig, &conf);
        conf.ePixelFormat = args->pixFormat;
        err = comp->SetParameter(comp, OMX_IndexParamVideoConfig, &conf);
        if (err != OMX_ErrorNone)
            goto FAIL;
    }
    ...
    if (args->secure)
    {
        ...
        conf.nPortIndex = 1; // output port
        comp->GetParameter(comp, OMX_IndexParamVideoConfig, &conf);
        conf.eDecMode = OMX_VIDEO_DEC_SECURITY;
        err = comp->SetParameter(comp, OMX_IndexParamVideoConfig, &conf);
        if (err != OMX_ErrorNone)
            goto FAIL;
    }

    if (args->cts_test && args->coding_type == OMX_VIDEO_CodingVP9)
    {
        ctsTest = 1;
        OMX_VIDEO_PARAM_CONFIGTYPE conf;
        INIT_OMX_TYPE(conf);
    ...
    if (args->adaptive)
    {
        if((args->coding_type == OMX_VIDEO_CodingHEVC) || (args->coding_type == OMX_VIDEO_CodingVP9))
        {
            ...
            conf.nPortIndex = 1; // output port
            comp->GetParameter(comp, OMX_IndexParamVideoConfig, &conf);
            conf.bEnableAdaptiveBuffers = OMX_TRUE;
            conf.nGuardSize = args->guard_buffers;
            err = comp->SetParameter(comp, OMX_IndexParamVideoConfig, &conf);
            if (err != OMX_ErrorNone)
                goto FAIL;
        }
...
//#endif
...
#else
    OMX_U32 i;
    // Push minimum required number of buffers to decoder
#if defined(LEADBUF_AT_ONCE)
    for (i=0; i<port.nBufferCountActual; ++i)
#else
    for (i=0; i<port.nBufferCountMin; ++i)
#endif
    ...
    while (eof == OMX_FALSE && ERROR == OMX_FALSE)
    {
        ...
        if (args->coding_type == OMX_VIDEO_CodingWMV && in_frame_count == 1)
        {
            vc1_highProfile = (input->pBuffer[0] == 0 && input->pBuffer[1] == 0 && input->pBuffer[2] == 1) ? 1 : 0;
            vc1_IsInterlaceSeq = (input->pBuffer[9] & 0x40) >> 6;
        }

        static first = 1;
        if(seek_flag && first)
        {
            if (args->coding_type == OMX_VIDEO_CodingHEVC)
            {
                u32 tmpret = HevcGetNalType((u8*)input->pBuffer, input->nAllocLen);
                while (tmpret == 0 && !eof)
                {
                    ret = rf(video, (u8*)input->pBuffer, input->nAllocLen, readstate, &eof);
                    if (ret < 0 || eof)
                        goto FAIL;
                    tmpret = HevcGetNalType((u8*)input->pBuffer, input->nAllocLen);
                }
            }
            else if (args->coding_type == OMX_VIDEO_CodingAVC)
            {
                u32 tmpret = H264GetNalType((u8*)input->pBuffer, input->nAllocLen);
                while (tmpret == 0 && !eof)
                {
                    ret = rf(video, (u8*)input->pBuffer, input->nAllocLen, readstate, &eof);
                    if (ret < 0 || eof)
                        goto FAIL;
                    tmpret = H264GetNalType((u8*)input->pBuffer, input->nAllocLen);
                }

            }
            else if (args->coding_type == OMX_VIDEO_CodingMPEG2)
            {
                u32 tmpret = Mpeg2GetStartCode((u8*)input->pBuffer, input->nAllocLen);
                ...
                while (tmpret == 0 && !eof)
                {
                    ...
                    if (ret < 0 || eof)
                        goto FAIL;
                    tmpret = Mpeg2GetStartCode((u8*)input->pBuffer, input->nAllocLen);
                }
            }
            else if (args->coding_type == OMX_VIDEO_CodingVP9)
            {
                u32 tmpret = Vp9IsKeyFrame((u8*)input->pBuffer, input->nAllocLen);
                ...
                while (tmpret == 0 && !eof)
                {
                    ...
                    if (ret < 0 || eof)
                        goto FAIL;
                    tmpret = Vp9IsKeyFrame((u8*)input->pBuffer, input->nAllocLen);
                }
            }
            else if (args->coding_type == OMX_VIDEO_CodingAVS)
            {
                u32 tmpret = AvsGetStartCode((u8*)input->pBuffer, input->nAllocLen);
                ...
                while (tmpret == 0 && !eof)
                {
                    ...
                    if (ret < 0 || eof)
                        goto FAIL;
                    tmpret = AvsGetStartCode((u8*)input->pBuffer, input->nAllocLen);
                }
            }
            else if (args->coding_type == OMX_VIDEO_CodingVP8)
            {
                u32 tmpret = Vp8IsKeyFrame((u8*)input->pBuffer, input->nAllocLen);
                ...
                while (tmpret == 0 && !eof)
                {
                    ...
                    if (ret < 0 || eof)
                        goto FAIL;
                    tmpret = Vp8IsKeyFrame((u8*)input->pBuffer, input->nAllocLen);
                }
            }
            else if (args->coding_type == OMX_VIDEO_CodingVP6)
            {
                u32 tmpret = Vp6IsKeyFrame((u8*)input->pBuffer, input->nAllocLen);
                ...
                while (tmpret == 0 && !eof)
                {
                    ...
                    if (ret < 0 || eof)
                        goto FAIL;
                    tmpret = Vp6IsKeyFrame((u8*)input->pBuffer, input->nAllocLen);
                }
            }
            else if (args->coding_type == OMX_VIDEO_CodingMPEG4 || args->coding_type == OMX_VIDEO_CodingDIVX)
            {
                u32 tmpret = Mpeg4GetStartCode((u8*)input->pBuffer, input->nAllocLen);
                ...
                while (tmpret == 0 && !eof)
                {
                    ...
                    if (ret < 0 || eof)
                        goto FAIL;
                    tmpret = Mpeg4GetStartCode((u8*)input->pBuffer, input->nAllocLen);
                }
            }
           else if (args->coding_type == OMX_VIDEO_CodingSORENSON)
            {
                u32 tmpret = SorensonGetStartCode((u8*)input->pBuffer, input->nAllocLen);
                ...
                while (tmpret == 0 && !eof)
                {
                    ...
                    if (ret < 0 || eof)
                        goto FAIL;
                    tmpret = SorensonGetStartCode((u8*)input->pBuffer, input->nAllocLen);
                }
            }
            else if (args->coding_type == OMX_VIDEO_CodingDIVX3)
            {
                u32 tmpret = Divx3GetStartCode((u8*)input->pBuffer, input->nAllocLen);
                ...
                while (tmpret == 0 && !eof)
                {
                    ...
                    if (ret < 0 || eof)
                        goto FAIL;
                    tmpret = Divx3GetStartCode((u8*)input->pBuffer, input->nAllocLen);
                }
            }
            else if (args->coding_type == OMX_VIDEO_CodingWMV)
            {
                u32 tmpret = VC1GetStartCode((u8*)input->pBuffer, input->nAllocLen);
                ...
                while (tmpret == 0 && !eof)
                {
                    ...
                    if (ret < 0 || eof)
                        goto FAIL;
                    tmpret = VC1GetStartCode((u8*)input->pBuffer, input->nAllocLen);
                }
            }
#ifdef ENABLE_CODEC_RV
            else if (args->coding_type == OMX_VIDEO_CodingRV)
            {
                u32 tmpret = RvGetStartCode((u8*)input->pBuffer, input->nAllocLen);
                ...
                while (tmpret == 0 && !eof)
                {
                    ...
                    if (ret < 0 || eof)
                        goto FAIL;
                    tmpret = RvGetStartCode((u8*)input->pBuffer, input->nAllocLen);
                }
            }
#endif
...
/* State transition: Executing -> Idle -> Loaded after FRAMECNT */

#if defined(STOP_BY_INBUFCOUNT)
        if (in_frame_count > FRAMECNT && !eof && reconfigPending == OMX_FALSE)
#else
        if (props->framecount >= FRAMECNT && !seek_flag && !eof && reconfigPending == OMX_FALSE)
#endif
        {
#ifdef TB_FLUSH
           printf("\nSend command OMX_CommandFlush\n");
           abort_flag = 1;
           ((OMX_COMPONENTTYPE*)dec)->SendCommand(dec, OMX_CommandFlush, OMX_ALL, NULL);
           ...
           while(queue_size(&outqueue) < port.nBufferCountActual)
		       usleep(VOUT_WEIGHT_USEC);

           init_list(&input_ret_queue);
           init_list(&input_queue);
           copy_list(&input_queue, &src_queue);
...
#if defined(TB_SEEK) || defined(TB_CHANGE_CHANNEL)
            printf("\nState change executing -> idle\n");
            abort_flag = 1;
            OMX_STATETYPE state = OMX_StateExecuting;

            ((OMX_COMPONENTTYPE*)dec)->GetState(dec, &state);
            if (state == OMX_StateExecuting)
            {
                ((OMX_COMPONENTTYPE*)dec)->SendCommand(dec, OMX_CommandStateSet, OMX_StateIdle, NULL);
                while (state != OMX_StateIdle)
                    ((OMX_COMPONENTTYPE*)dec)->GetState(dec, &state);
                printf("Now idle\n");
                ...
                printf("\nState change idle -> executing\n");

                if (state == OMX_StateIdle)
                ((OMX_COMPONENTTYPE*)dec)->SendCommand(dec, OMX_CommandStateSet, OMX_StateExecuting, NULL);
...
#ifdef TB_CHANGE_CHANNEL
                printf("\nState change idle -> loaded\n");
                if (state == OMX_StateIdle)
                ((OMX_COMPONENTTYPE*)dec)->SendCommand(dec, OMX_CommandStateSet, OMX_StateLoaded, NULL);

#ifdef ENABLE_CODEC_RV
                if(threads[0]) // cancel real file format reader thread
                    pthread_cancel(threads[0]);
#endif
                printf("Free buffers\n");
                if (alpha_mask_buffer)
                    ((OMX_COMPONENTTYPE*)dec)->FreeBuffer(dec, 2, alpha_mask_buffer);
                ...
                for (i=0; i<INPUT_BUFFER_COUNT; ++i)
                {
                    ...
                    if (src_hdr)
                    {
                        printf("Freeing Buffer(in) %d (%p).\n", i, src_hdr);
                        ((OMX_COMPONENTTYPE*)dec)->FreeBuffer(dec, 0, src_hdr);
                    }
                }
                ...
                for (i=0; i<port.nBufferCountActual; ++i)
                {
                    ...
                    if (dst_hdr) {
                        printf("Freeing Buffer(out) %d (%p).\n", i, dst_hdr);

                        ((OMX_COMPONENTTYPE*)dec)->FreeBuffer(dec, 1, dst_hdr);
                    }
                }
                ...
                printf("Now idle. Wait 1 seconds and goto destructor\n");
                sleep(1);

                ret = !ERROR;
                goto DESTRUCTOR; // not fail
//                return 0;
#endif
            }
#endif
...
#if 0
            OMX_STATETYPE state;
            ((OMX_COMPONENTTYPE*)dec)->SendCommand(dec, OMX_CommandStateSet, OMX_StatePause, NULL);
            while (state != OMX_StatePause && state != OMX_StateInvalid) {
                usleep(1000);
                ((OMX_COMPONENTTYPE*)dec)->GetState(dec, &state);
            }
            ((OMX_COMPONENTTYPE*)dec)->SendCommand(dec, OMX_CommandStateSet, OMX_StateExecuting, NULL);
...
#if defined(USE_VOUT_TH)
    run_voutth = OMX_FALSE;
    queue_abort(&outqueue);
    if(vout_thread) {// join real file format reader thread
        pthread_join(vout_thread, &thread_ret);
    }
#endif
...
int compare_output(const char* reference, const char* temp)
{
    ...
    else
    {
        ...
        if (buff_tmp == NULL || buff_ref == NULL)
        {
            goto FAIL;
        }
...
int map_format_string(const char* format)
{
    ...
    for (; i <= OMX_COLOR_FormatYUV420SemiPlanarP010; ++i)
    {
        if (strcmp(name, HantroOmx_str_omx_color(i))==0)
            return i;
    }
...
int parse_argument_vector(const char** argv, int argc, TESTARGS* args)
{
    ...
    if (args->name == NULL)
    {
        DBGT_CRITICAL("args->name is NULL");
        return -1;
    }
    for(i=0; i<argc; ++i)
    {
        ...
        else if (!strcmp(argv[i], "-ib"))
        {
            ...
            if (args->buffer_size_in == 0 ||
                args->buffer_size_in > DEFAULT_BUFFER_SIZE_INPUT) {
                args->buffer_size_in = DEFAULT_BUFFER_SIZE_INPUT;
                DBGT_CRITICAL("Invalid buffer_size_in, set to default value %d", args->buffer_size_in);
                return -1;
            }
        }
        else if (!strcmp(argv[i], "-ob"))
        {
            ...
            if (args->buffer_size_out == 0 ||
                args->buffer_size_out > DEFAULT_BUFFER_SIZE_OUTPUT) {
                args->buffer_size_out = DEFAULT_BUFFER_SIZE_OUTPUT;
                DBGT_CRITICAL("Invalid buffer_size_out, set to default value %d", args->buffer_size_out);
                return -1;
            }
        }
        else if (!strcmp(argv[i], "-id"))
        {
            ...
            if (args->test_id) free(args->test_id);
        ...
        else if (!strcmp(argv[i], "-bi"))
        {
            ASSURE_NEXT_ARG(i, argc);
            const char* arg = argv[++i];
            if (args->blend_input) free(args->blend_input);
        ...
        else if (!strcmp(argv[i], "-rf"))
        {
            ASSURE_NEXT_ARG(i, argc);
            const char* arg = argv[++i];
            if (args->reference_file) free(args->reference_file);
        ...
        else if (!strcmp(argv[i], "-rd"))
        {
            ASSURE_NEXT_ARG(i, argc);
            const char* arg = argv[++i];
            if (args->reference_dir) free(args->reference_dir);
...
TEST_RESULT execute_test(const TESTARGS* test)
{
    ...
    if (func && decode_file(test->input, output, &props, test, func, &rcv))
    {
...
int main(int argc, const char* argv[])
{
    ...
    err = OSAL_MutexCreate(&realloc_mutex);
    if (err != OMX_ErrorNone)
    {
        DBGT_CRITICAL("Mutex create error: %s", HantroOmx_str_omx_err(err));
        return 1;
...
#if defined(USE_VOUT_TH)
    queue_init(&outqueue, sizeof(FILLDONEARGS));
    run_voutth = OMX_TRUE;
    pthread_create(&vout_thread, NULL, (void *)vout_thread_main, NULL);
#endif
    ...
    if (argc > 2)
    {
        ...
        else
        {
            ...
            if (test.blend_input) free(test.blend_input);
            if (test.name)    free(test.name);
            if (test.output_file) free(test.output_file);
            return 1;
        }
    }
    else
...
u8 HevcGetNalType(u8 *pByteStream, u32 len)
{
    ...
    if (naltype == 19 || naltype == 20 || naltype == 32 || naltype == 33)
        return 1;
...
u8 Mpeg2GetStartCode(u8 *pByteStream, u32 len)
{
    ...
    if(startCode == 0xB3 || startCode == 0xB5 || startCode == 0xB8)
    {
        printf("start code  = %d\n", startCode);
        return 1;
    }
    ...
    if(startCode == 0x00)
    {
        pByteStream = pByteStream + 2;
        if(((*pByteStream & 0x38) >> 3) == 1)
        {
            printf("start code  = %d\n", startCode);
            return 1;
        }
...
u8 Vp9IsKeyFrame(u8 *pByteStream, u32 len)
{
    u32 vpVersion = (*pByteStream & 0x20) >> 5;
    u32 vpProfile = (*pByteStream & 0x10) >> 3 + vpVersion;
    u32 showExistingFrame;
    u32 frameType = 1;
    if(vpProfile > 2)
    {
        showExistingFrame = (*pByteStream & 0x04) >> 2;
        if(!showExistingFrame)
            frameType = (*pByteStream & 0x02) >> 1;
    }
    else
    {
        showExistingFrame = (*pByteStream & 0x08) >> 3;
        if(!showExistingFrame)
            frameType = (*pByteStream & 0x04) >> 2;
    }
...
u8 AvsGetStartCode(u8 *pByteStream, u32 len)
{
    ...
    if(startCode == 0xB3 || startCode == 0xB0)
    {
        printf("start code  = %d\n", startCode);
        return 1;
    }
...
u8 Mpeg4GetStartCode(u8 *pByteStream, u32 len)
{
    ...
    if(startCode == 0xB0 || startCode == 0x00 || startCode == 0x20 || startCode == 0xB3)
    {
        printf("start code  = %d\n", startCode);
        return 1;
    }
...
u8 SorensonGetStartCode(u8 *pByteStream, u32 len)
{
    u32 codingType = -1;
    u8 *temp = pByteStream;
    u32 sourceFormat = (((*(temp+3)) & 0x03) << 1) + ((*(temp+4) >> 7));
    if(sourceFormat == 0)
        codingType = (*(pByteStream+6) & 0x30) >> 5;
    else if(sourceFormat == 1)
        codingType = (*(pByteStream+8) & 0x30) >> 5;
    else
        codingType = (*(pByteStream+4) & 0x30) >> 5;
    if(codingType == 0)
        return 1;
    else
        return 0;
...
#define FRAME_INTERLACE 1
#define FIELD_INTERLACE 2
u8 VC1GetStartCode(u8 *pByteStream, u32 len)
{
    u32 startCode;
    u32 tmp;
    u32 fcm;
    u32 picType;
    startCode = *(pByteStream + 3);
    if (startCode == 0x0E || startCode == 0x0F)
        return 1;

    if (startCode == 0x0D )
    {
       if (vc1_IsInterlaceSeq)
       {
           if ((*(pByteStream + 4) >> 7) > 0)
               fcm = ((*(pByteStream + 4) & 0x40) >> 6) > 0 ? FIELD_INTERLACE : FRAME_INTERLACE;
           else
               fcm = PROGRESSIVE;
       }
       else
       ...
       if (vc1_IsInterlaceSeq && fcm == FIELD_INTERLACE)
       {
           picType = (*(pByteStream + 4) & 0x38) >> 3;
           if(picType <= 1)
               return 1;
           else
               return 0;
       ...
       else
       {
           if(vc1_highProfile)
           {
               if (vc1_IsInterlaceSeq &&  fcm == FRAME_INTERLACE)
                   tmp = (*(pByteStream + 4) & 0x38) >> 3;
               if (vc1_IsInterlaceSeq &&  fcm == PROGRESSIVE)
                   tmp = (*(pByteStream + 4) & 0x70) >> 4;
               if (!vc1_IsInterlaceSeq)
                   tmp = (*(pByteStream + 4) & 0xE0) >> 5;
           ...
           {
               if (vc1_IsInterlaceSeq &&  fcm == FRAME_INTERLACE)
                   tmp = (*(pByteStream + 4) & 0x30) >> 4;
               if (vc1_IsInterlaceSeq &&  fcm == PROGRESSIVE)
                   tmp = (*(pByteStream + 4) & 0x60) >> 5;
               if (!vc1_IsInterlaceSeq)
                   tmp = (*(pByteStream + 4) & 0xD0) >> 6;
...
u8 RvGetStartCode(u8 *pByteStream, u32 len1)
{
    u32 startCode;
    u32 bits;
    u32 len = 0;
    ...
    if (rv.eFormat == OMX_VIDEO_RVFormat9 && rawRVFile != OMX_TRUE)
        startCode = (*pByteStream & 0x60) >> 5 ;
    if (rv.eFormat == OMX_VIDEO_RVFormat8 && rawRVFile != OMX_TRUE)
        startCode = (*pByteStream & 0x18) >> 3 ;

    if(rawRVFile)
    {
        if (rv.eFormat == OMX_VIDEO_RVFormat9)
            pByteStream += 4;
        if (rv.eFormat == OMX_VIDEO_RVFormat8)
            pByteStream += 3;
        do
        {
            info = rvVlcTable[(u8)(*pByteStream <<(tmp)) + (*(pByteStream+1) >> (8-tmp))];
            chunkLen = info>>4;
    ...
    if (startCode == 0x00 || startCode == 0x01)
        return 1;
