...
#        rm -f mb_control.bin rlc.bin

        refencode
        mv stream.mpeg4 $case_dir/reference.mpeg4

        echo "Input from directory $case_dir"

        # Run encoder
        encode
        mv stream.mpeg4 $case_dir/output.mpeg4
#        cat encoder_mpeg4.log

        if [ -s "$case_dir/output.mpeg4" ]; then
            cmp $case_dir/output.mpeg4 $case_dir/reference.mpeg4
            comparison_result=$?

            if [ $comparison_result -gt 0 ]; then
                if [ -s "$MPEGDEC" ]; then
                    $MPEGDEC -Oref.yuv $case_dir/reference.mpeg4 >> ref.log
                    $MPEGDEC -Oomx.yuv $case_dir/output.mpeg4 >> omx.log
            ...
            comparison_result=1
            echo "failed, $case_dir/output.mpeg4 size 0 or file not found"
        fi
