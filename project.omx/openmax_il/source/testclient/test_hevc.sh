...
resultfile=results_hevc
#HEVCDEC=hevcdec
# Current date
date=`date +"%d.%m.%y %k:%M"`

#if [ -s "test_data_parameter_hevc.sh" ] ; then
#    TEST_PARAMETER=test_data_parameter_hevc.sh
#else
    TEST_PARAMETER=customer_data_parameter_hevc.sh
#fi
...
ref_enc_bin=$SYSTEM_MODEL_HOME/hevc_testenc
...
refencode() {
	...
	--intraPicRate=${intraVopRate[${set_nro}]} \
...
#        omxLastVop="$[${lastVop[${set_nro}]}+1]"
    ...
    omxnPFrames=0
    if [ ${intraVopRate[${set_nro}]} -gt 0 ] ; then
        omxnPFrames="$[${intraVopRate[${set_nro}]}-1]"
    else
        omxnPFrames=149
    fi
    ...
    case "${inputFormat[$set_nro]}" in
        2|3|4|5|6|7|8|9) {
            let inputFormat[$set_nro]=${inputFormat[$set_nro]}-1
              };;
        * )
    esac
    ...
    case "${inputFormat[$set_nro]}" in
        ...
        3|4|5|6|7|8|9|10) {
            #ycbycr | cbycry | rgb565 | bgr565 | rgb555 | bgr555 | rgb444 | brg444
...
compare() {
    if [ -s "$case_dir/output.hevc" ]; then
        if cmp $case_dir/output.hevc $case_dir/reference.hevc; then
            ...
            if [ -s "$HEVCDEC" ]; then
                $HEVCDEC -Oref.yuv $case_dir/reference.hevc >> ref.log
                $HEVCDEC -Oomx.yuv $case_dir/output.hevc >> omx.log
        ...
        echo "Case $case_nro FAILED"
        echo "E case_$case_nro;SW/HW Integration;;$date;Failed;;$hwtag;$USER;;;$swtag;$systag"
        echo "failed, $case_dir/output.hevc size 0 or file not found"
    fi
}
