                 ...
                 u32 nuit)
{
  ...
  sei->hdr10_display_enable  = (u32)ENCHW_NO;
  sei->hdr10_dx0     = 0;
  sei->hdr10_dy0     = 0;
  sei->hdr10_dx1     = 0;
  sei->hdr10_dy1     = 0;
...
/*-----------------------------------------------------------------------------------------------------
HevcUpdateSeiMasteringDisplayColour()  upadate info about Mastering display colour volume SEI message.
------------------------------------------------------------------------------------------------------*/
void HevcUpdateSeiMasteringDisplayColour(sei_s *sei, Hdr10DisplaySei *pDisplaySei)
{
	...
	sei->hdr10_display_enable = pDisplaySei->hdr10_display_enable;
	sei->hdr10_dx0 = pDisplaySei->hdr10_dx0;
	sei->hdr10_dy0 = pDisplaySei->hdr10_dy0;
	sei->hdr10_dx1 = pDisplaySei->hdr10_dx1;
	sei->hdr10_dy1 = pDisplaySei->hdr10_dy1;
	sei->hdr10_dx2 = pDisplaySei->hdr10_dx2;
	sei->hdr10_dy2 = pDisplaySei->hdr10_dy2;
	sei->hdr10_wx = pDisplaySei->hdr10_wx;
	sei->hdr10_wy = pDisplaySei->hdr10_wy;
	sei->hdr10_maxluma = pDisplaySei->hdr10_maxluma;
	sei->hdr10_minluma = pDisplaySei->hdr10_minluma;
}
...
void HevcMasteringDisplayColourSei(struct buffer *sp, sei_s *sei)
{
	...
	put_bit_32(sp, sei->hdr10_dx0, 16);
	COMMENT(sp, "display_primaries_x_c0");
	put_bit_32(sp, sei->hdr10_dy0, 16);
	COMMENT(sp, "display_primaries_y_c0");
	
	put_bit_32(sp, sei->hdr10_dx1, 16);
	COMMENT(sp, "display_primaries_x_c1");
	put_bit_32(sp, sei->hdr10_dy1, 16);
	COMMENT(sp, "display_primaries_y_c1");
	put_bit_32(sp, sei->hdr10_dx2, 16);
	COMMENT(sp, "display_primaries_x_c2");
	put_bit_32(sp, sei->hdr10_dy2, 16);
	COMMENT(sp, "display_primaries_y_c2");

	put_bit_32(sp, sei->hdr10_wx, 16);
	COMMENT(sp, "white_point_x");
	put_bit_32(sp, sei->hdr10_wy, 16);
	COMMENT(sp, "white_point_y");

	put_bit_32(sp, sei->hdr10_maxluma, 32);
	COMMENT(sp, "max_display_mastering_luminance");
	put_bit_32(sp, sei->hdr10_minluma, 32);
...
/*-----------------------------------------------------------------------------------------------------
HevcUpdateSeiContentLightLevelInfo()  upadate info about Content light level info SEI message.
------------------------------------------------------------------------------------------------------*/
void HevcUpdateSeiContentLightLevelInfo(sei_s *sei, Hdr10LightLevelSei *pLightSei)
{
	...
	sei->hdr10_lightlevel_enable = pLightSei->hdr10_lightlevel_enable;
	sei->hdr10_maxlight = pLightSei->hdr10_maxlight;
	sei->hdr10_avglight = pLightSei->hdr10_avglight;
}
...
void HevcContentLightLevelSei(struct buffer *sp, sei_s *sei)
{
	...
	put_bit_32(sp, sei->hdr10_maxlight, 16);
	COMMENT(sp, "max_content_light_level");
	put_bit_32(sp, sei->hdr10_avglight, 16);
...
void H264PicTimingSei(       struct buffer * sp, sei_s * sei)
{
    ...
    if(sei->hrd)
    {
        put_bit_32(sp, sei->crd, sei->crdLen);
...
void H264RecoveryPointSei(struct buffer * sp, sei_s * sei)
{
    ...
    put_bit_se(sp, sei->recoveryFrameCnt-1);
