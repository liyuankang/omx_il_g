...
/*------------------------------------------------------------------------------
  create_picture
------------------------------------------------------------------------------*/
struct sw_picture *create_picture_ctrlsw(struct vcenc_instance *vcenc_instance, struct vps *vps, struct sps *sps,
    struct pps *pps, u32 sliceSize, u32 srcWidth, u32 srcHeight)
{
  ...
  w = (srcWidth + 15) &(~15);
  h = srcHeight;
  if (sw_init_image(p, &p->input, w, h, SW_IMAGE_U8, HANTRO_FALSE, 0, 0)) goto out;
  ...
  if (sw_init_image(p, &p->recon, w, h, SW_IMAGE_U8, HANTRO_FALSE, 0, 0)) goto out;
...
void reference_picture_list(struct container *c, struct sw_picture *pic)
{
  ...
  for (n = pic->slice.tail; n; n = n->next)
  {
    ...
    if ((pic->sliceInst->type != I_SLICE && s->active_l0_cnt != pic->pps->num_ref_idx_l0_default_active) ||
        (pic->sliceInst->type == B_SLICE && s->active_l1_cnt != pic->pps->num_ref_idx_l1_default_active))
  ...
  {
    s = (struct slice *)n;
    s->active_override_flag = HANTRO_TRUE || flag || (cnt[0] != s->active_l0_cnt) || (cnt[1] != s->active_l1_cnt);
