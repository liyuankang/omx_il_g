...
void HevcConfigureTestBeforeFrame(struct vcenc_instance *inst)
{
  ...
  switch (inst->testId)
  {
    ...
    case TID_LTR: HevcLtrIntervalTest(inst);                break;
    case TID_SMT: VCEncSmartModeTest(inst);         break;
    case TID_CCH: VCEncConstChromaTest(inst);         break;
    case TID_RC: VCEncCtbRcTest(inst);         break;
    default: break;
  }
}
...
void HevcConfigureTestBeforeStart(struct vcenc_instance *inst)
{
  ...
  switch (inst->testId)
  {
    ...
    case TID_INT: HevcStreamBufferLimitTest(inst, NULL);        break;
    case 21: HevcInterFavorTest(inst);              break;
    case 23: HevcDMVPenaltyTest(inst);              break;
    case 31: HevcPenaltyTest(inst);                 break;
    case TID_IPCM: HevcIPCMTest(inst);                     break;
    case TID_GMV: VCEncGMVTest(inst);                 break;
    case TID_EXTLINEBUF: VCEncExtLineBufferTest(inst);                 break;
    case TID_MEVR: VCEncMEVertRangeTest(inst);                 break;
    default: break;
  }
}
...
void HevcFrameQuantizationTest(struct vcenc_instance *inst)
{
  ...
  inst->rateControl.qpHdr = MIN(inst->rateControl.qpMaxPB,
...
void HevcStreamBufferLimitTest(struct vcenc_instance *inst, VCEncStrmBufs *bufs)
{
  ...
  if (swctrl->asicCfg.streamBufferChain && swctrl->outputStrmSize[1] && (inst->parallelCoreNum <= 1))
  {
    ...
    printf("HevcStreamBufferLimitTest# streamBuffer Addr %p %p Size %d + %d = %d bytes\n",
        (u8 *)swctrl->outputStrmBase[0], (u8 *)swctrl->outputStrmBase[1], size0, size1, size0+size1);
...
void HevcRoiTest(struct vcenc_instance *inst)
{
  ...
  if(regs->asicCfg.ROI8Support)
  {
      printf("HevcRoiTest# ROI3:%d x%dy%d-x%dy%d  ROI4:%d x%dy%d-x%dy%d\n",
         regs->roi3DeltaQp, regs->roi3Left, regs->roi3Top,
         regs->roi3Right, regs->roi3Bottom,
         regs->roi4DeltaQp, regs->roi4Left, regs->roi4Top,
         regs->roi4Right, regs->roi4Bottom);

      printf("HevcRoiTest# ROI5:%d x%dy%d-x%dy%d  ROI6:%d x%dy%d-x%dy%d\n",
         regs->roi5DeltaQp, regs->roi5Left, regs->roi5Top,
         regs->roi5Right, regs->roi5Bottom,
         regs->roi6DeltaQp, regs->roi6Left, regs->roi6Top,
         regs->roi6Right, regs->roi6Bottom);

      printf("HevcRoiTest# ROI7:%d x%dy%d-x%dy%d  ROI8:%d x%dy%d-x%dy%d\n",
         regs->roi7DeltaQp, regs->roi7Left, regs->roi7Top,
         regs->roi7Right, regs->roi7Bottom,
         regs->roi8DeltaQp, regs->roi8Left, regs->roi8Top,
...
void HevcIPCMTest(struct vcenc_instance *inst)
{
  ...
  printf("HevcIPCMTest# IPCM1: x%dy%d-x%dy%d  IPCM2: x%dy%d-x%dy%d\n",
         regs->ipcm1AreaLeft, regs->ipcm1AreaTop,
         regs->ipcm1AreaRight, regs->ipcm1AreaBottom,
         regs->ipcm2AreaLeft, regs->ipcm2AreaTop,
...
void HevcRFDTest(struct vcenc_instance *inst, struct sw_picture *pic)
{
  ...
  if(pic->recon_compress.lumaCompressed) {
    u8 *lum_table = (u8 *)(asic->compressTbl[pic->picture_memeory_id].virtualAddress);
    u8 *lum = (u8 *)(asic->internalreconLuma[pic->picture_memeory_id].virtualAddress);
    if(pic->poc & 1) {
  ...
  if(pic->recon_compress.chromaCompressed) {
    ...
    u8 *chroma_table = ((u8 *)(asic->compressTbl[pic->picture_memeory_id].virtualAddress)) + lumaTblSize;
...
void VCEncCtbRcTest(struct vcenc_instance *inst)
{
  ...
  printf("CtbRcTest# tolctbrcinter = %f, tolctbrcintra = %f, deltaRange = %d\n",
          inst->rateControl.tolCtbRcInter, inst->rateControl.tolCtbRcIntra, inst->rateControl.rcQpDeltaRange);
}
...
void VCEncGMVTest(struct vcenc_instance *inst)
{
  ...
  if (regs->frameCodingType != 1) printf("GmvRcTest# List0 GMV = (%d %d)\n", regs->gmv[0][0], regs->gmv[0][1]);
  if (regs->frameCodingType == 2) printf("GmvRcTest# List1 GMV = (%d %d)\n", regs->gmv[1][0], regs->gmv[1][1]);
}
...
{
  regValues_s *regs = &inst->asic.regs;
  if ((!regs->asicCfg.meVertRangeProgramable) || (inst->frameCnt==0) || (regs->frameCodingType==1)) return;

  i32 rangeH264[4] = {24, 48, 24, 0};
