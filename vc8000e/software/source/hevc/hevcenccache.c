...
i32 VCEncEnableCache(struct vcenc_instance *vcenc_instance,asicData_s *asic,struct sw_picture *pic)
{
  i32 ret = CACHE_OK;
  char buf[18];
  ...
  if (vcenc_instance->preProcess.inputFormat != VCENC_YUV420_SEMIPLANAR &&
      ...
      vcenc_instance->preProcess.inputFormat != VCENC_YUV420_SEMIPLANAR_8BIT_FB &&
  ...
  if (vcenc_instance->inputLineBuf.inputLineBufLoopBackEn==0)
  {
    switch (vcenc_instance->preProcess.inputFormat)
    {
      ...
      case VCENC_YUV420_SEMIPLANAR_VU:
        ...
        channel_cfg.trace_end_addr = STRIDE(vcenc_instance->preProcess.lumWidthSrc,alignment)*vcenc_instance->height;
        
        channel_cfg.start_addr = pic->input.lum>>4;
        channel_cfg.end_addr = (pic->input.lum +(STRIDE(vcenc_instance->preProcess.lumWidthSrc,alignment)*vcenc_instance->preProcess.lumHeightSrc))>>4;
      ...
      case VCENC_YUV420_PLANAR_10BIT_P010:
        ...
        channel_cfg.trace_start_addr = BaseInLum;
        channel_cfg.trace_end_addr = STRIDE(vcenc_instance->preProcess.lumWidthSrc*2,alignment)*vcenc_instance->preProcess.lumHeightSrc;
        
        channel_cfg.start_addr = pic->input.lum>>4;
        channel_cfg.end_addr = (pic->input.lum+(STRIDE(vcenc_instance->preProcess.lumWidthSrc*2,alignment)*vcenc_instance->preProcess.lumHeightSrc))>>4;
      ...
      case VCENC_YUV420_PLANAR_10BIT_P010_FB:
        ...
        if (vcenc_instance->preProcess.inputFormat==VCENC_YUV420_SEMIPLANAR_8BIT_FB || vcenc_instance->preProcess.inputFormat==VCENC_YUV420_SEMIPLANAR_VU_8BIT_FB)
          byte_per_compt = 1;
        else if (vcenc_instance->preProcess.inputFormat==VCENC_YUV420_PLANAR_10BIT_P010_FB)
          byte_per_compt = 2;
        
        channel_cfg.trace_start_addr = BaseInLum;
        channel_cfg.trace_end_addr = (STRIDE(vcenc_instance->preProcess.lumWidthSrc*4*byte_per_compt,alignment)*vcenc_instance->preProcess.lumHeightSrc/4);
        
        channel_cfg.start_addr = pic->input.lum>>4;
        channel_cfg.end_addr = (pic->input.lum+(STRIDE(vcenc_instance->preProcess.lumWidthSrc*4*byte_per_compt,alignment)*vcenc_instance->preProcess.lumHeightSrc/4))>>4;
  ...
  if (vcenc_instance->inputLineBuf.inputLineBufLoopBackEn==0)
  {
    switch (vcenc_instance->preProcess.inputFormat)
    {
      ...
      case VCENC_YUV420_SEMIPLANAR_VU:
        ...
        channel_cfg.trace_end_addr = STRIDE(vcenc_instance->preProcess.lumWidthSrc,alignment)*vcenc_instance->height/2;
    
        channel_cfg.start_addr = asic->regs.inputCbBase>>4;
        channel_cfg.end_addr = (asic->regs.inputCbBase +(STRIDE(vcenc_instance->preProcess.lumWidthSrc,alignment)*vcenc_instance->preProcess.lumHeightSrc/2))>>4;
      ...
      case VCENC_YUV420_PLANAR_10BIT_P010:
        ...
        channel_cfg.trace_start_addr = BaseInCb;
        channel_cfg.trace_end_addr = STRIDE(vcenc_instance->preProcess.lumWidthSrc*2,alignment)*vcenc_instance->height/2;
    
        channel_cfg.start_addr = asic->regs.inputCbBase>>4;
        channel_cfg.end_addr = (asic->regs.inputCbBase +(STRIDE(vcenc_instance->preProcess.lumWidthSrc*2,alignment)*vcenc_instance->preProcess.lumHeightSrc/2))>>4;
      ...
      case VCENC_YUV420_PLANAR_10BIT_P010_FB:
        ...
        channel_cfg.trace_start_addr = BaseInCb;
        channel_cfg.trace_end_addr = STRIDE(vcenc_instance->preProcess.lumWidthSrc*4*byte_per_compt,alignment)*((vcenc_instance->preProcess.lumHeightSrc/2)+3)/4;
    
        channel_cfg.start_addr = asic->regs.inputCbBase>>4;
        channel_cfg.end_addr = (asic->regs.inputCbBase +(STRIDE(vcenc_instance->preProcess.lumWidthSrc*4*byte_per_compt,alignment)*((vcenc_instance->preProcess.lumHeightSrc/2)+3)/4))>>4;
  ...
  byte_per_compt = vcenc_instance->asic.regs.P010RefEnable+1;
    ...
    u32 reference_luma_size = asic->regs.ref_frame_stride * (((vcenc_instance->height + 63)&(~63))/(8/byte_per_compt));
#else
    u32 reference_luma_size = asic->regs.ref_frame_stride * ((vcenc_instance->height + 63)&(~63))/4;
#endif

  u32 reference_4n_size = asic->regs.ref_ds_luma_stride * ((vcenc_instance->height + 63)&(~63))/4/4;
  ...
  if (pic->sliceInst->type != I_SLICE)
  {
    ...
    j = 0;
    while ((i32)asic->regs.pRefPic_recon_l0[0][0] != refLum[j++]) ASSERT(j < NUM_REF_BUF);

    channel_cfg.trace_start_addr = traceRefLu[j];
    channel_cfg.trace_end_addr = ((vcenc_instance->width + alignment)&(~alignment))*((vcenc_instance->height + 63)&(~63));
  ...
  if (pic->sliceInst->type != I_SLICE)
  {
    ...
    j = 0;
    while ((i32)asic->regs.pRefPic_recon_l0[1][0] != refChr[j++]) ASSERT(j < NUM_REF_BUF);

    channel_cfg.trace_start_addr = traceRefChr[j];   
    channel_cfg.trace_end_addr = ((vcenc_instance->width + alignment)&(~alignment))*((vcenc_instance->height + 63)&(~63))/2;
  ...
  if (pic->sliceInst->type == B_SLICE)
  {
    ...
    j = 0;
    while ((i32)asic->regs.pRefPic_recon_l1[0][0] != refLum[j++]) ASSERT(j < NUM_REF_BUF);
  ...
  if (pic->sliceInst->type == B_SLICE)
  {
    ...
    j = 0;
    while ((i32)asic->regs.pRefPic_recon_l1[1][0] != refChr[j++]) ASSERT(j < NUM_REF_BUF);
  ...
  if (pic->sliceInst->type != I_SLICE)
  {
    ...
    j = 0;
    while ((i32)asic->regs.pRefPic_recon_l0_4n[0] != ref4nLum[j++]) ASSERT(j < NUM_REF_BUF);

    channel_cfg.trace_start_addr = traceRef4N[j];
    channel_cfg.trace_end_addr = ((vcenc_instance->height/4 + alignment)&(~alignment))*vcenc_instance->height/4;
  ...
  if (pic->sliceInst->type == B_SLICE)
  {
    ...
    j = 0;
    while ((i32)asic->regs.pRefPic_recon_l1_4n[0] != ref4nLum[j++]) ASSERT(j < NUM_REF_BUF);
...
VCEncRet EnableCache(struct vcenc_instance *vcenc_instance, asicData_s *asic,struct sw_picture *pic)
{
    ...
    if (CACHE_OK != VCEncEnableCache(vcenc_instance,asic,pic))
    {
      printf("Encoder enable cache failed!!\n");
      return VCENC_ERROR;
    }
    
    EnableCacheWork(vcenc_instance->cache);
...
void DisableCache(struct vcenc_instance *vcenc_instance)
{
    ...
    DisableCacheChannelALL(&vcenc_instance->cache,CACHE_BI);
