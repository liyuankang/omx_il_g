...
struct sps sps =
{
  ...
  /* AV1 definations*/
  .still_picture = ENCHW_NO,
  .reduced_still_picture_hdr = ENCHW_NO,
  .timing_info_present_flag = ENCHW_NO,
  .decoder_model_info_present_flag = ENCHW_NO,
  .initial_display_delay_present_flag = ENCHW_NO,
  .use_128x128_superblock = ENCHW_NO,
  .enable_filter_intra = ENCHW_NO,
  .enable_intra_edge_filter = ENCHW_NO,
  .frame_id_numbers_present_flag = ENCHW_NO,
  .enable_interintra_compound = ENCHW_NO,
  .enable_masked_compound = ENCHW_NO,
  .enable_warped_motion = ENCHW_NO,
  .enable_dual_filter = ENCHW_NO,
  .enable_order_hint = ENCHW_YES,
  .enable_jnt_comp = ENCHW_NO,
  .enable_ref_frame_mvs = ENCHW_NO,
  .choose_screen_content_tools = ENCHW_NO,
  .force_screen_content_tools = ENCHW_NO,
  .force_integer_mv           = ENCHW_NO,
  .enable_superres = ENCHW_NO,
  .enable_cdef = ENCHW_NO,
  .enable_restoration = ENCHW_NO,
  .film_grain_params_present = ENCHW_NO,
  .separate_uv_delta_q = ENCHW_NO,

  .operating_points_cnt_minus1 = 0,
  .order_hint_bits             = DEFAULT_EXPLICIT_ORDER_HINT_BITS,
 // .frame_id_length;
...
static const u8 list0[16] =
{
  16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16
};

static const u8 list1[64] =
{
...
static const i32 forward_scaling[6] =
{
  26214, 23302, 20560, 18396, 16384, 14564
};

static const i32 inverse_scaling[6] =
{
  40, 45, 51, 57, 64, 72
};

static void profile_and_level(struct buffer *b, i32 max_num_sub_layers, u32 profile,u32 level, u32 tier, VCEncInst inst);
...
i32 init_parameter_set(struct sps *s, struct pps *p)
{
  ...
  */
  ...
  /* Tiles ... */
  if (tile_init(p, p->tiles_enabled_flag, p->num_tile_columns, p->num_tile_rows)) goto out;
...
i32 set_reference_pic_set(struct rps *r)
{
  ...
  r->ref_pic_lt = qalloc(m, r->num_lt_pics, sizeof(struct ref_pic));
...
void video_parameter_set(struct vps *v, VCEncInst inst)
{
  ...
  profile_and_level(b, v->max_num_sub_layers,v->general_profile_idc, v->general_level_idc, v->general_tier_flag, inst);
...
/*------------------------------------------------------------------------------
  profile_and_level
------------------------------------------------------------------------------*/
void profile_and_level(struct buffer *b, i32 max_num_sub_layers, u32 profile,u32 level, u32 tier, VCEncInst inst)
{
  struct vcenc_instance *vcenc_instance = (struct vcenc_instance *)inst;
...
static void HEVCSeqParameterSet(struct container *c, struct sps *s, VCEncInst inst)
{
  ...
  profile_and_level(b, s->max_num_sub_layers, s->general_profile_idc,s->general_level_idc, s->general_tier_flag, inst);
...
------------------------------------------------------------------------------*/
void init_poc_list(struct rps *r, i32 poc, bool use_ltr_cur, u32 codingType, struct container *c, bool bH264, const bool *pLTR_used)
{
  struct ref_pic *p;
  i32 tmp, i, j, k = 0, ii;
  i32 lt_poc[4];
  i32 long_term_used[4] = {0, 0, 0, 0};
  ...
  // TODO: When long-term pic appears in r->follow[]:
  // 1. mark it as used_by_curr_pic & insert into before[]/after[]; Or
  // 2. remove it from follow and mark as long-term reference.
  // Both methods need slice level RPS update.
  r->after_cnt = j;
  r->follow_cnt = k;

  j = 0;

  if((codingType == VCENC_INTRA_FRAME)&&(poc>0)&&(r->before_cnt ==1)&&(r->follow_cnt==0))
  {
...
void VCEncSpsSetVuiSignalType(struct sps *sps, u32 VideoSignalTypePresentFlag, u32 video_format, u32 videoFullRange,
                                               u32 ColorDescripPresentFlag, u32 ColorPrimaries, u32 TransferCharacteristics, u32 MatrixCoefficients)
{
	ASSERT(sps);

	sps->vui.VideoSignalTypePresentFlag = VideoSignalTypePresentFlag;
	sps->vui.video_format   = video_format;
	sps->vui.videoFullRange = videoFullRange;

	sps->vui.ColorDescripPresentFlag = ColorDescripPresentFlag;
	sps->vui.ColorPrimaries = ColorPrimaries;
	sps->vui.TransferCharacteristics = TransferCharacteristics;
	sps->vui.MatrixCoefficients = MatrixCoefficients;

	if( (VideoSignalTypePresentFlag == ENCHW_YES) || (ColorDescripPresentFlag == ENCHW_YES))
	    sps->vui_parameters_present_flag = ENCHW_YES;
}
...
static void WriteVui(struct buffer *b, vui_t *vui)
{
  ...
  if (vui->VideoSignalTypePresentFlag == ENCHW_YES)
  {
    put_bit(b, 1, 1);
	...
	put_bit(b, vui->videoFullRange, 1);
    COMMENT(b, "video_full_range_flag");

    put_bit(b, vui->ColorDescripPresentFlag, 1);
    COMMENT(b, "colour_description_present_flag");
	if (vui->ColorDescripPresentFlag == ENCHW_YES)
	{
		put_bit(b, vui->ColorPrimaries, 8);
		COMMENT(b, "Color Primaries");

		put_bit(b, vui->TransferCharacteristics, 8);
		COMMENT(b, "Transfer Characteristics");

		put_bit(b, vui->MatrixCoefficients, 8);
...
void UpdateVuiPresence(struct sps *sps)
{
  ...
  if (sps->vui.nalHrdParametersPresentFlag == 0 &&
	  ...
	  sps->vui.sarWidth == 0 && sps->vui.videoFullRange == 0 && sps->vui.ColorDescripPresentFlag == 0 &&
...
/* Code */
    ...
    else
    {
        ...
        if(sarIdc == EXTENDED_SAR)
        {
            put_bit(b, vui->sarWidth, 16);
            COMMENT(b, "sar_width");
            put_bit(b, vui->sarHeight, 16);
            COMMENT(b, "sar_height");
        }
    }
...
{
    H264NalUnitHdr(b, 0, H264_ENDOFSEQUENCE, (byte_stream==VCENC_BYTE_STREAM) ? ENCHW_YES : ENCHW_NO);
    rbsp_flush_bits(b);
}
