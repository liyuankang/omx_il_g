...
#define av1_sw_copy(dest, src)              \
  {                                      \
    ASSERT(sizeof(dest) == sizeof(src)); \
    memcpy(dest, src, sizeof(src));      \
  }
static const u16 default_kf_y_mode_cdf[KF_MODE_CONTEXTS][KF_MODE_CONTEXTS][CDF_SIZE(INTRA_MODES)] = {
...
static const u16 default_angle_delta_cdf[DIRECTIONAL_MODES][CDF_SIZE( 2 * MAX_ANGLE_DELTA + 1)] = {
...
static const u16 default_if_y_mode_cdf[BLOCK_SIZE_GROUPS][CDF_SIZE(INTRA_MODES)] = { 
...
static const u16 default_uv_mode_cdf[CFL_ALLOWED_TYPES][INTRA_MODES][CDF_SIZE(UV_INTRA_MODES)] = {
...
static const u16 default_partition_cdf[PARTITION_CONTEXTS][CDF_SIZE(EXT_PARTITION_TYPES)] = {
...
static const u16 default_intra_ext_tx_cdf[EXT_TX_SETS_INTRA][EXT_TX_SIZES][INTRA_MODES][CDF_SIZE(TX_TYPES)] = {
      {
          {
              { 0 },
              { 0 },
...
static const u16 default_inter_ext_tx_cdf[EXT_TX_SETS_INTER][EXT_TX_SIZES][CDF_SIZE(TX_TYPES)] = {
      {
          { 0 },
          { 0 },
          { 0 },
...
static const u16 default_switchable_interp_cdf[SWITCHABLE_FILTER_CONTEXTS][CDF_SIZE(AV1_BILINEAR)] = {
...
static const u16 default_inter_compound_mode_cdf[INTER_MODE_CONTEXTS][CDF_SIZE(INTER_COMPOUND_MODES)] = {
...
static const u16 default_uni_comp_ref_cdf[UNI_COMP_REF_CONTEXTS][UNIDIR_COMP_REFS - 1][CDF_SIZE(2)] = {
...
static const u16 default_palette_y_size_cdf[PALATTE_BSIZE_CTXS][CDF_SIZE(PALETTE_SIZES)] = {
...
static const u16 default_palette_uv_size_cdf[PALATTE_BSIZE_CTXS][CDF_SIZE(PALETTE_SIZES)] = {
...
static const u16 default_palette_y_mode_cdf[PALATTE_BSIZE_CTXS][PALETTE_Y_MODE_CONTEXTS][CDF_SIZE(2)] = {
...
static const u16 default_palette_y_color_index_cdf[PALETTE_SIZES][PALETTE_COLOR_INDEX_CONTEXTS][CDF_SIZE(PALETTE_COLORS)] = {
      {
          { AOM_CDF2(28710) },
          { AOM_CDF2(16384) },
          { AOM_CDF2(10553) },
...
static const u16 default_palette_uv_color_index_cdf[PALETTE_SIZES][PALETTE_COLOR_INDEX_CONTEXTS][CDF_SIZE(PALETTE_COLORS)] = {
      {
          { AOM_CDF2(29089) },
          { AOM_CDF2(16384) },
          { AOM_CDF2(8713) },
...
static const u16 default_switchable_restore_cdf[CDF_SIZE(RESTORE_SWITCHABLE_TYPES)] = 
     { AOM_CDF3(9413, 22581) };

static const u16 default_wiener_restore_cdf[CDF_SIZE(2)] = 
     { AOM_CDF2(11570) };

static const u16 default_sgrproj_restore_cdf[CDF_SIZE(2)] = 
     { AOM_CDF2(16855) };
...
static const u16 default_seg_tree_cdf[CDF_SIZE(MAX_SEGMENTS)] = {
...
static const u16 default_spatial_pred_seg_tree_cdf[SPATIAL_PREDICTION_PROBS][CDF_SIZE(MAX_SEGMENTS)] = {
      {
...
static const u16 default_tx_size_cdf[MAX_TX_CATS][TX_SIZE_CONTEXTS][CDF_SIZE(MAX_TX_DEPTH + 1)] = {
                                               { { AOM_CDF2(19968) },
                                                 { AOM_CDF2(19968) },
                                                 { AOM_CDF2(24320) } },
...
static const u16 default_mv_class_context[2][CDF_SIZE(MV_CLASSES)] = {
                                                     // Vertical component
...
void av1_default_coef_probs(int qp, FRAME_CONTEXT *fc) {
  const int index = get_q_ctx(quantizer_to_qindex[qp]);

  av1_sw_copy(fc->txb_skip_cdf, av1_default_txb_skip_cdfs[index]);
  av1_sw_copy(fc->eob_extra_cdf, av1_default_eob_extra_cdfs[index]);
  av1_sw_copy(fc->dc_sign_cdf, av1_default_dc_sign_cdfs[index]);
  av1_sw_copy(fc->coeff_br_cdf, av1_default_coeff_lps_multi_cdfs[index]);
  av1_sw_copy(fc->coeff_base_cdf, av1_default_coeff_base_multi_cdfs[index]);
  av1_sw_copy(fc->coeff_base_eob_cdf,av1_default_coeff_base_eob_multi_cdfs[index]);
  av1_sw_copy(fc->eob_flag_cdf16, av1_default_eob_multi16_cdfs[index]);
  av1_sw_copy(fc->eob_flag_cdf32, av1_default_eob_multi32_cdfs[index]);
  av1_sw_copy(fc->eob_flag_cdf64, av1_default_eob_multi64_cdfs[index]);
  av1_sw_copy(fc->eob_flag_cdf128, av1_default_eob_multi128_cdfs[index]);
  av1_sw_copy(fc->eob_flag_cdf256, av1_default_eob_multi256_cdfs[index]);
  av1_sw_copy(fc->eob_flag_cdf512, av1_default_eob_multi512_cdfs[index]);
  av1_sw_copy(fc->eob_flag_cdf1024, av1_default_eob_multi1024_cdfs[index]);
}

static void init_mode_probs(FRAME_CONTEXT *fc) {
  //av1_sw_copy(fc->palette_y_size_cdf, default_palette_y_size_cdf);
  //av1_sw_copy(fc->palette_uv_size_cdf, default_palette_uv_size_cdf);
  //av1_sw_copy(fc->palette_y_color_index_cdf, default_palette_y_color_index_cdf);
  //av1_sw_copy(fc->palette_uv_color_index_cdf, default_palette_uv_color_index_cdf);
  av1_sw_copy(fc->kf_y_cdf, default_kf_y_mode_cdf);
  av1_sw_copy(fc->angle_delta_cdf, default_angle_delta_cdf);
  av1_sw_copy(fc->comp_inter_cdf, default_comp_inter_cdf);
  av1_sw_copy(fc->comp_ref_type_cdf, default_comp_ref_type_cdf);
  av1_sw_copy(fc->uni_comp_ref_cdf, default_uni_comp_ref_cdf);
  //av1_sw_copy(fc->palette_y_mode_cdf, default_palette_y_mode_cdf);
  //av1_sw_copy(fc->palette_uv_mode_cdf, default_palette_uv_mode_cdf);
  av1_sw_copy(fc->comp_ref_cdf, default_comp_ref_cdf);
  av1_sw_copy(fc->comp_bwdref_cdf, default_comp_bwdref_cdf);
  av1_sw_copy(fc->single_ref_cdf, default_single_ref_cdf);
  av1_sw_copy(fc->txfm_partition_cdf, default_txfm_partition_cdf);
  av1_sw_copy(fc->compound_index_cdf, default_compound_idx_cdfs);
  av1_sw_copy(fc->comp_group_idx_cdf, default_comp_group_idx_cdfs);
  av1_sw_copy(fc->newmv_cdf, default_newmv_cdf);
  av1_sw_copy(fc->zeromv_cdf, default_zeromv_cdf);
  av1_sw_copy(fc->refmv_cdf, default_refmv_cdf);
  av1_sw_copy(fc->drl_cdf, default_drl_cdf);
  av1_sw_copy(fc->motion_mode_cdf, default_motion_mode_cdf);
  av1_sw_copy(fc->obmc_cdf, default_obmc_cdf);
  av1_sw_copy(fc->inter_compound_mode_cdf, default_inter_compound_mode_cdf);
  av1_sw_copy(fc->compound_type_cdf, default_compound_type_cdf);
  av1_sw_copy(fc->wedge_idx_cdf, default_wedge_idx_cdf);
  av1_sw_copy(fc->interintra_cdf, default_interintra_cdf);
  av1_sw_copy(fc->wedge_interintra_cdf, default_wedge_interintra_cdf);
  av1_sw_copy(fc->interintra_mode_cdf, default_interintra_mode_cdf);
  //av1_copy(fc->seg.pred_cdf, default_segment_pred_cdf);
  //av1_copy(fc->seg.tree_cdf, default_seg_tree_cdf);
  av1_sw_copy(fc->filter_intra_cdfs, default_filter_intra_cdfs);
  av1_sw_copy(fc->filter_intra_mode_cdf, default_filter_intra_mode_cdf);
  av1_sw_copy(fc->switchable_restore_cdf, default_switchable_restore_cdf);
  av1_sw_copy(fc->wiener_restore_cdf, default_wiener_restore_cdf);
  av1_sw_copy(fc->sgrproj_restore_cdf, default_sgrproj_restore_cdf);
  av1_sw_copy(fc->y_mode_cdf, default_if_y_mode_cdf);
  av1_sw_copy(fc->uv_mode_cdf, default_uv_mode_cdf);
  av1_sw_copy(fc->switchable_interp_cdf, default_switchable_interp_cdf);
  av1_sw_copy(fc->partition_cdf, default_partition_cdf);
  av1_sw_copy(fc->intra_ext_tx_cdf, default_intra_ext_tx_cdf);
  av1_sw_copy(fc->inter_ext_tx_cdf, default_inter_ext_tx_cdf);
  av1_sw_copy(fc->skip_mode_cdfs, default_skip_mode_cdfs);
  av1_sw_copy(fc->skip_cdfs, default_skip_cdfs);
  av1_sw_copy(fc->intra_inter_cdf, default_intra_inter_cdf);
  //for (int i = 0; i < SPATIAL_PREDICTION_PROBS; i++)
  //  av1_copy(fc->seg.spatial_pred_seg_cdf[i], default_spatial_pred_seg_tree_cdf[i]);
  av1_sw_copy(fc->tx_size_cdf, default_tx_size_cdf);
  av1_sw_copy(fc->delta_q_cdf, default_delta_q_cdf);
  av1_sw_copy(fc->delta_lf_cdf, default_delta_lf_cdf);
  av1_sw_copy(fc->delta_lf_multi_cdf, default_delta_lf_multi_cdf);
  av1_sw_copy(fc->cfl_sign_cdf, default_cfl_sign_cdf);
  av1_sw_copy(fc->cfl_alpha_cdf, default_cfl_alpha_cdf);
  av1_sw_copy(fc->intrabc_cdf, default_intrabc_cdf);
}

void asic_av1_init_mv_probs(FRAME_CONTEXT *fc) {
    av1_sw_copy(fc->mv_classes_cdf, default_mv_class_context);
    // NB: this sets CDFs too
    fc->nmvc = default_nmv_context;
    //fc->ndvc = default_nmv_context;
}

void av1_prob_init(int qp, FRAME_CONTEXT *fc){
  // reset mode ref deltas
  // av1_set_default_ref_deltas(cm->cur_frame->ref_deltas);
  // av1_set_default_mode_deltas(cm->cur_frame->mode_deltas);
  // set_default_lf_deltas(&cm->lf);

  av1_default_coef_probs(qp, fc);
  init_mode_probs(fc);
  asic_av1_init_mv_probs(fc);
}
