...
u32 EncPreGetHwFormat(u32 inputFormat)
{
  ...
  static const u32 inputFormatMapping[37] =
  {
    ...
    ASIC_INPUT_DAHUA_HEVC_YUV420,
    ASIC_INPUT_DAHUA_H264_YUV420,
    ASIC_INPUT_FB_YUV420SEMIPLANAR,
    ASIC_INPUT_FB_YUV420SEMIPLANAR,
    ASIC_INPUT_FB_P010,
...
static const u32 rgbMaskBits[37][3] =
{
  {  0,  0,  0 },
  {  0,  0,  0 },
  {  0,  0,  0 },
...
u32 EncGetAlignedByteStride(int width, i32 input_format, u32 *luma_stride, u32 *chroma_stride,u32 input_alignment)
{
  ...
  switch (input_format)
  {
    ...
    case 18://VCENC_YUV420_10BIT_PACKED_Y0L2
      ...
      *chroma_stride = 0;
      break;
    case 19://VCENC_YUV420_PLANAR_8BIT_DAHUA_HEVC
      *luma_stride = STRIDE(width,32);
      *chroma_stride = STRIDE(width,32)/2;
      break;
    case 20://VCENC_YUV420_PLANAR_8BIT_DAHUA_H264
      *luma_stride = STRIDE(width,16);
      *chroma_stride = 0;
      break;
    case 21://VCENC_YUV420_SEMIPLANAR_8BIT_FB
    case 22://VCENC_YUV420_SEMIPLANAR_VU_8BIT_FB
      *luma_stride = STRIDE(width*4,alignment);
      *chroma_stride = STRIDE(width*4,alignment);
      break;
    case 23://VCENC_YUV420_PLANAR_10BIT_P010_FB
...
void EncPreProcess(asicData_s *asic, preProcess_s *preProcess)
{
  ...
  pixelByte = EncGetAlignedByteStride(preProcess->lumWidthSrc, preProcess->inputFormat, &luma_stride, &chroma_stride, alignment);
  ...
  {
   if (preProcess->bottomField) horOffsetSrc += luma_stride;
   luma_stride *= 2;
  }
  ...
  if (preProcess->inputFormat <= 2)   /* YUV 420 planar/semiplanar */
  {
    ...
    if (preProcess->inputFormat == 0)
    {
      ...
      regs->dummyReadAddr = regs->inputCrBase + regs->inputChromaBaseOffset + height64/2*chroma_stride + preProcess->lumWidth/2-1;
    }
    else
    {
      tmp = preProcess->verOffsetSrc / 2;
      ...
      regs->inputCbBase += (tmp & (~15));
      regs->inputChromaBaseOffset = tmp & 15;
      regs->dummyReadAddr = regs->inputCbBase + regs->inputChromaBaseOffset + height64/2*chroma_stride + preProcess->lumWidth-1;
    }
  }
  else if (preProcess->inputFormat <= 10)   /* YUV 422 / RGB 16bpp */
  {
    ...
    if(regs->inputImageFormat==ASIC_INPUT_YUYV422INTERLEAVED||regs->inputImageFormat==ASIC_INPUT_UYVY422INTERLEAVED)
    ...
    regs->dummyReadAddr = regs->inputLumBase + regs->inputLumaBaseOffset + height64*luma_stride*2 + preProcess->lumWidth*2-1;
  ...
  else if(preProcess->inputFormat <= 14)    /* RGB 32bpp */
  {
    ...
    regs->inputLumaBaseOffset = (tmp & 15);
    regs->inputChromaBaseOffset = (regs->inputLumaBaseOffset / 4) * 4;
    regs->dummyReadAddr = regs->inputLumBase + regs->inputLumaBaseOffset + height64*luma_stride + preProcess->lumWidth*4-1;
  ...
  else if(preProcess->inputFormat == 15)//I010
  {
    ...
    regs->dummyReadAddr = regs->inputCrBase + regs->inputChromaBaseOffset + height64/2*chroma_stride*2 + preProcess->lumWidth-1;
  }
  else if(preProcess->inputFormat == 16)//P010
  {
    ...
    regs->dummyReadAddr = regs->inputCbBase + regs->inputChromaBaseOffset + (height64/2*luma_stride/2 + preProcess->lumWidth/2)*4-1;
  }
  else if(preProcess->inputFormat == 17)
  {
    ...
    regs->dummyReadAddr = regs->inputCrBase + regs->inputChromaBaseOffset + height64/2*luma_stride*5/8 + (preProcess->lumWidth)*5/8-1;
  }
  ...
  else if (preProcess->inputFormat == 19)   /* YUV 420 dahua  for hevc */
  {
    /* Input image position after crop and stabilization */
  ...
  else if (preProcess->inputFormat == 20)   /* YUV 420 dahua  for h264 */
  {
    /* Input image position after crop and stabilization */
    /*crop alignment for 32x32*/
    u32 mb_per_row = preProcess->lumWidthSrc/16;
	u32 mb_cropped = mb_per_row*preProcess->verOffsetSrc/16+horOffsetSrc/16;

    tmp = mb_cropped/5*2048+mb_cropped%5*400;

	if (preProcess->sliced_frame)//jpeg dahua h264
	{
	  u32 slice_cropped = mb_per_row*preProcess->verOffsetSrc/16;
    ...
    regs->inputCrBase = preProcess->verOffsetSrc/16;
  ...
  else if (preProcess->inputFormat == 21||preProcess->inputFormat == 22||preProcess->inputFormat == 23)
  {
    regs->dummyReadAddr = regs->inputCbBase + (preProcess->verOffsetSrc+height64)/8*luma_stride + ((preProcess->horOffsetSrc+preProcess->lumWidth)*4-1)*(1+(preProcess->inputFormat == 23));
  }
  ...
  EWLHwConfig_t config = EncAsicGetAsicConfig(0);
      ...
      preProcess->scaledOutput)
  {
    ...
    else
    {
      ...
      {
        regs->scaledSkipTopPixelRow=(((height8*regs->scaledHeightRatio)>>16)-preProcess->scaledHeight)/2;
      ...
      {
        regs->scaledSkipLeftPixelColumn=(((width8*regs->scaledWidthRatio)>>16)-preProcess->scaledWidth)/2;
  ...
  u32 mcuh = (regs->codingType == ASIC_JPEG && regs->jpegMode == 1 && regs->ljpegFmt == 1 ? 8 : 16);
  ...
  if((regs->codingType == ASIC_JPEG && !(regs->jpegMode == 1 && regs->ljpegFmt == 1)) || (regs->codingType == ASIC_H264)) {
    if (height & 0x0f)
