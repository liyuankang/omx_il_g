...
#define COST_BOOL(prob, bin)\
  ((bin) ? vp9_prob_cost[255 - (prob)] : vp9_prob_cost[prob])
...
------------------------------------------------------------------------------*/
static void InitDeadzone(vp9Instance_s *inst);
static i32 CostTree(tree const *tree, i32 *prob);
static void UpdateEntropy(vp9Instance_s *inst);
...
/*------------------------------------------------------------------------------
    InitDeadzone
------------------------------------------------------------------------------*/
void InitDeadzone(vp9Instance_s *inst)
{
  entropy *entropy = inst->entropy;
...
------------------------------------------------------------------------------*/
void InitTreePenaltyTables(vp9Instance_s *container)
{
...
i32 CostMv(i32 mvd, i32 *mvProb)
{
  ...
  /* Luma motion vectors are doubled, see 18.1 in "VP9 Data Format and
   * Decoding Guide". */
  ASSERT(!(mvd & 1));
  tmp = ABS(mvd >> 1);
    ...
    for (j = 0; j < 8; j++)
    {
      for (k = 0; k < 3; k++)
      {
        for (l = 0; l < 11; l++)
        {
          ...
          if (new == old)
          {
            VP9PutBool(buffer, prob, 0);
            //COMMENT("Coeff prob update");
          }
          else
          {
            VP9PutBool(buffer, prob, 1);
            //COMMENT("Coeff prob update");
            VP9PutLit(buffer, new, 8);
            //COMMENT("New prob");
          }
        }
      }
...
u32 update(u32 updP, u32 left, u32 right, u32 oldP, u32 newP, u32 fixed)
{
  ...
  u = (i32)fixed + ((vp9_prob_cost[255 - updP] - vp9_prob_cost[updP]) >> 8);
  /* bit savings if updated */
  s = ((i32)left * /* zero branch count */
       /* diff cost for '0' bin */
       (vp9_prob_cost[oldP] - vp9_prob_cost[newP]) +
       (i32)right * /* one branch count */
       /* diff cost for '1' bin */
       (vp9_prob_cost[255 - oldP] - vp9_prob_cost[255 - newP])) >> 8;
...
/*------------------------------------------------------------------------------
  UpdateEntropy
------------------------------------------------------------------------------*/
void UpdateEntropy(vp9Instance_s *inst)
{
  entropy *entropy = inst->entropy;
  i32 i, j, k, l, tmp, ii;
  ...
  if ((ENCH2_VP9_ENTROPY_UPDATE_DISTANCE > 1) &&
      (inst->frameCnt % ENCH2_VP9_ENTROPY_UPDATE_DISTANCE) != 0) return;

#ifdef TRACE_PROBS
  /* Trace HW output prob counters into file */
  EncTraceProbs(pCnt, ASIC_VP9_PROB_COUNT_SIZE);
#endif
  ...
  /* skip prob */
  pTmp = pCnt + ASIC_VP9_PROB_COUNT_MODE_OFFSET;
  ...
  /* mv probs */
  pTmp = pCnt + ASIC_VP9_PROB_COUNT_MV_OFFSET;
  for (i = 0; i < 2; i++)
  {
    /* is short prob */
    left = *pTmp++; /* short */
