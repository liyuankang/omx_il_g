...
------------------------------------------------------------------------------*/
static void Vp9FrameQuantizationTest(vp9Instance_s *inst);
static void Vp9DeadzoneTest(vp9Instance_s *inst);
static void Vp9StreamBufferLimitTest(vp9Instance_s *inst);
static void Vp9FilterTest(vp9Instance_s *inst);
static void Vp9FilterDeltaTest(vp9Instance_s *inst);
static void Vp9FilterDeltaDisable(vp9Instance_s *inst);
static void Vp9SegmentTest(vp9Instance_s *inst);
static void Vp9SegmentMapTest(vp9Instance_s *inst);
static void Vp9RefFrameTest(vp9Instance_s *inst);
static void Vp9TemporalLayerTest(vp9Instance_s *inst);
static void Vp9ZeroMvFavorTest(vp9Instance_s *inst);
static void Vp9Intra16FavorTest(vp9Instance_s *inst);
static void Vp9InterFavorTest(vp9Instance_s *inst);
static void Vp9CroppingTest(vp9Instance_s *inst);
static void Vp9RgbInputMaskTest(vp9Instance_s *inst);
static void Vp9MvTest(vp9Instance_s *inst);
static void Vp9DMVPenaltyTest(vp9Instance_s *inst);
static void Vp9MaxOverfillMv(vp9Instance_s *inst);
static void Vp9RoiTest(vp9Instance_s *inst);
static void Vp9IntraAreaTest(vp9Instance_s *inst);
static void Vp9CirTest(vp9Instance_s *inst);
static void Vp9IntraModeTest(vp9Instance_s *inst);
static void Vp9SkipPenaltyTest(vp9Instance_s *inst);
static void Vp9PenaltyTest(vp9Instance_s *inst);
static void Vp9IntraSliceRefreshTest(vp9Instance_s *inst);
static void Vp9QuantDeltaTest(vp9Instance_s *inst);
static void Vp9DownscalingTest(vp9Instance_s *inst);
...
void Vp9ConfigureTestBeforeFrame(vp9Instance_s *inst)
{
  ...
  switch (inst->testId)
  {
    case 1: Vp9FrameQuantizationTest(inst);     break;
    case 4: Vp9StreamBufferLimitTest(inst);     break;
    case 5: Vp9FilterDeltaDisable(inst);        break;
    case 6: Vp9FilterDeltaTest(inst);           break;
    case 7: Vp9FilterTest(inst);                break;
    case 8: Vp9SegmentTest(inst);               break;
    case 9: Vp9RefFrameTest(inst);              break;
    case 10: Vp9SegmentMapTest(inst);           break;
    case 11: Vp9TemporalLayerTest(inst);        break;
    case 16: Vp9CroppingTest(inst);             break;
    case 19: Vp9RgbInputMaskTest(inst);         break;
    case 22: Vp9MvTest(inst);                   break;
    case 24: Vp9MaxOverfillMv(inst);            break;
    case 26: Vp9RoiTest(inst);                  break;
    case 27: Vp9IntraAreaTest(inst);            break;
    case 28: Vp9CirTest(inst);                  break;
    case 32: Vp9IntraSliceRefreshTest(inst);    break;
    case 33: Vp9QuantDeltaTest(inst);           break;
    case 34: Vp9DownscalingTest(inst);          break;
    default: break;
  }
...
void Vp9ConfigureTestPenalties(vp9Instance_s *inst)
{
  ...
  switch (inst->testId)
  {
    case 3:  Vp9DeadzoneTest(inst);             break;
    case 14: Vp9ZeroMvFavorTest(inst);          break;
    case 15: Vp9Intra16FavorTest(inst);         break;
    case 21: Vp9InterFavorTest(inst);           break;
    case 23: Vp9DMVPenaltyTest(inst);           break;
    case 30: Vp9SkipPenaltyTest(inst);          break;
    case 31: Vp9PenaltyTest(inst);              break;
    default: Vp9IntraModeTest(inst);            break;
  }
...
/*------------------------------------------------------------------------------
  Vp9QuantizationTest
------------------------------------------------------------------------------*/
void Vp9FrameQuantizationTest(vp9Instance_s *inst)
{
  ...
  printf("Vp9FrameQuantTest# qpHdr %d\n", inst->rateControl.qpHdr);
}

/*------------------------------------------------------------------------------
  Vp9DeadzoneTest
------------------------------------------------------------------------------*/
void Vp9DeadzoneTest(vp9Instance_s *inst)
{
#if 0
  inst->asic.regs.deadzoneEnable = 1;
  inst->asic.regs.pen[0][ASIC_PENALTY_DZ_SKIP1] = 0x3FF;

  printf("Vp9DeadzoneTest# deadzone=%d RateSkip=%d (max skip cost)\n",
...
/*------------------------------------------------------------------------------
  Vp9StreamBufferLimitTest
------------------------------------------------------------------------------*/
void Vp9StreamBufferLimitTest(vp9Instance_s *inst)
{
  ...
  printf("Vp9StreamBufferLimitTest# streamBufferLimit %d bytes, Y-offset %d\n",
...
/*------------------------------------------------------------------------------
  Vp9FilterTest
------------------------------------------------------------------------------*/
void Vp9FilterTest(vp9Instance_s *inst)
{
  i32 vopNum = inst->frameCnt;
  sps *sps = &inst->sps;
  ...
  printf("Vp9FilterTest# filterType = %d filterLevel = %i filterSharpnesss = %i\n",
...
/*------------------------------------------------------------------------------
  Vp9FilterDeltaTest
------------------------------------------------------------------------------*/
void Vp9FilterDeltaTest(vp9Instance_s *inst)
{
  ...
  printf("Vp9FilterDeltaTest# refDelta = %d,%d,%d,%d  modeDelta = %d,%d,%d,%d\n",
...
/*------------------------------------------------------------------------------
  Vp9FilterDeltaDisable
------------------------------------------------------------------------------*/
void Vp9FilterDeltaDisable(vp9Instance_s *inst)
{
  sps *sps = &inst->sps;

  sps->filterDeltaEnable = 0;

  printf("Vp9FilterDeltaDisable#\n");
}

/*------------------------------------------------------------------------------
  Vp9SegmentTest
------------------------------------------------------------------------------*/
void Vp9SegmentTest(vp9Instance_s *inst)
{
  ...
  printf("Vp9SegmentTest# enable=%d update=%d map=0x%08x%08x%08x%08x\n",
         pps->segmentEnabled, pps->sgm.mapModified,
         map[0], map[1], map[2], map[3]);
  printf("Vp9SegmentTest# qp=%d,%d,%d,%d level=%d,%d,%d,%d\n",
...
/*------------------------------------------------------------------------------
  Vp9SegmentMapTest
------------------------------------------------------------------------------*/
void Vp9SegmentMapTest(vp9Instance_s *inst)
{
  u32 frame = (u32)inst->frameCnt;
  u32 *map = inst->asic.segmentMap.virtualAddress;
  sps *sps = &inst->sps;
  ...
  printf("Vp9SegmentMapTest# enable=%d update=%d map=0x%08x%08x%08x%08x\n",
         pps->segmentEnabled, pps->sgm.mapModified,
         map[0], map[1], map[2], map[3]);
  printf("Vp9SegmentMapTest# qp=%d,%d,%d,%d level=%d,%d,%d,%d\n",
         pps->qpSgm[0], pps->qpSgm[1], pps->qpSgm[2], pps->qpSgm[3],
         pps->levelSgm[0], pps->levelSgm[1],
         pps->levelSgm[2], pps->levelSgm[3]);
}
...
/*------------------------------------------------------------------------------
  Vp9RefFrameTest
------------------------------------------------------------------------------*/
void Vp9RefFrameTest(vp9Instance_s *inst)
{
  ...
  printf("Vp9RefFrameTest#\n");
}

/*------------------------------------------------------------------------------
  Vp9TemporalLayerTest
------------------------------------------------------------------------------*/
void Vp9TemporalLayerTest(vp9Instance_s *inst)
{
  i32 pic = inst->frameCnt;
  picBuffer *picBuffer = &inst->picBuffer;
  ...
  printf("Vp9TemporalLayerTest#\n");
}

/*------------------------------------------------------------------------------
  Vp9ZeroMvFavorTest
------------------------------------------------------------------------------*/
void Vp9ZeroMvFavorTest(vp9Instance_s *inst)
{
  u32 frame = (u32)inst->frameCnt;
#if 0
  ...
  printf("Vp9ZeroMvFavorTest# zeroMvFavorDiv2 %d\n",
...
/*------------------------------------------------------------------------------
  Vp9Intra16FavorTest
------------------------------------------------------------------------------*/
void Vp9Intra16FavorTest(vp9Instance_s *inst)
{
  i32 s;
  ...
  printf("Vp9Intra16FavorTest# intra16Favor %d\n",
...
/*------------------------------------------------------------------------------
  Vp9InterFavorTest
------------------------------------------------------------------------------*/
void Vp9InterFavorTest(vp9Instance_s *inst)
{
  u32 frame = (u32)inst->frameCnt;
  i32 s;
#if 0
  ...
  printf("Vp9InterFavorTest# interFavor %d skipPenalty %d goldenPenalty %d\n",
...
/*------------------------------------------------------------------------------
  Vp9CroppingTest
------------------------------------------------------------------------------*/
void Vp9CroppingTest(vp9Instance_s *inst)
{
  ...
  printf("Vp9CroppingTest# horOffsetSrc %d  verOffsetSrc %d\n",
...
/*------------------------------------------------------------------------------
  Vp9RgbInputMaskTest
------------------------------------------------------------------------------*/
void Vp9RgbInputMaskTest(vp9Instance_s *inst)
{
  ...
  printf("Vp9RgbInputMaskTest#  %d %d %d\n", inst->asic.regs.rMaskMsb,
...
/*------------------------------------------------------------------------------
  Vp9MvTest
------------------------------------------------------------------------------*/
void Vp9MvTest(vp9Instance_s *inst)
{
  u32 frame = (u32)inst->frameCnt;
  ...
  printf("Vp9MvTest# horOffsetSrc %d  verOffsetSrc %d\n",
         inst->preProcess.horOffsetSrc, inst->preProcess.verOffsetSrc);
}
...
/*------------------------------------------------------------------------------
  Vp9DMVPenaltyTest
------------------------------------------------------------------------------*/
void Vp9DMVPenaltyTest(vp9Instance_s *inst)
{
  u32 frame = (u32)inst->frameCnt;
  i32 s;
#if 0
  ...
  printf("Vp9DMVPenaltyTest# penalty4p %d  penalty1p %d  penaltyQp %d\n",
...
/*------------------------------------------------------------------------------
  Vp9MaxOverfillMv
------------------------------------------------------------------------------*/
void Vp9MaxOverfillMv(vp9Instance_s *inst)
{
  u32 frame = (u32)inst->frameCnt;
  ...
  printf("Vp9MaxOverfillMv# horOffsetSrc %d  verOffsetSrc %d\n",
         inst->preProcess.horOffsetSrc, inst->preProcess.verOffsetSrc);
}
...
/*------------------------------------------------------------------------------
  Vp9RoiTest
------------------------------------------------------------------------------*/
void Vp9RoiTest(vp9Instance_s *inst)
{
  ...
  printf("Vp9RoiTest# ROI1:%d x%dy%d-x%dy%d  ROI2:%d x%dy%d-x%dy%d\n",
...
/*------------------------------------------------------------------------------
  Vp9IntraAreaTest
------------------------------------------------------------------------------*/
void Vp9IntraAreaTest(vp9Instance_s *inst)
{
  regValues_s *regs = &inst->asic.regs;
  u32 frame = (u32)inst->frameCnt;
  u32 mbPerRow = inst->mbPerRow;
  ...
  printf("Vp9IntraAreaTest# x%dy%d-x%dy%d\n",
         regs->intraAreaLeft, regs->intraAreaTop,
         regs->intraAreaRight, regs->intraAreaBottom);
}

/*------------------------------------------------------------------------------
  Vp9IntraSliceRefreshTest
------------------------------------------------------------------------------*/
void Vp9IntraSliceRefreshTest(vp9Instance_s *inst)
{
  regValues_s *regs = &inst->asic.regs;
  u32 frame = (u32)inst->frameCnt;
  u32 mbPerRow = inst->mbPerRow;
  ...
  printf("Vp9IntraSliceRefreshTest# rowsPerSlice %d  y%d-y%d\n",
...
/*------------------------------------------------------------------------------
  Vp9CirTest
------------------------------------------------------------------------------*/
void Vp9CirTest(vp9Instance_s *inst)
{
  regValues_s *regs = &inst->asic.regs;
  u32 frame = (u32)inst->frameCnt;
  u32 mbPerRow = inst->mbPerRow;
  ...
  printf("Vp9CirTest# start:%d interval:%d\n",
...
/*------------------------------------------------------------------------------
  Vp9IntraModeTest
------------------------------------------------------------------------------*/
void Vp9IntraModeTest(vp9Instance_s *inst)
{
  regValues_s *regs = &inst->asic.regs;
  u32 frame = (u32)inst->frameCnt;
  i32 i, s;
  ...
  if (inst->testId >= 80 && inst->testId <= 94)
    printf("Vp9IntraModeTest# %d\n", inst->testId - 80);
#endif
}
/*------------------------------------------------------------------------------
  Vp9SkipPenaltyTest
------------------------------------------------------------------------------*/
void Vp9SkipPenaltyTest(vp9Instance_s *inst)
{
  u32 frame = (u32)inst->frameCnt;
  i32 s;
#if 0
  ...
  printf("Vp9SkipPenaltyTest# skipPenalty %d\n",
         inst->asic.regs.pen[0][ASIC_PENALTY_SKIP]);
#endif
}
...
/*------------------------------------------------------------------------------
  Vp9QuantDeltaTest
------------------------------------------------------------------------------*/
void Vp9QuantDeltaTest(vp9Instance_s *inst)
{
  u32 frame = (u32)inst->frameCnt;
  sps *sps = &inst->sps;
  i32 i;
  ...
  printf("Vp9QuantDeltaTest# QP Delta [0..4] = %d %d %d %d %d\n",
...
/*------------------------------------------------------------------------------
  Vp9PenaltyTest
------------------------------------------------------------------------------*/
void Vp9PenaltyTest(vp9Instance_s *inst)
{
#if 0
  i32 s;
  ...
  printf("Vp9PenaltyTest# skipPenalty %d  zeroMvFavorDiv2 %d\n",
         inst->asic.regs.pen[0][ASIC_PENALTY_SKIP],
         inst->asic.regs.zeroMvFavorDiv2);
#endif
}
...
/*------------------------------------------------------------------------------
  Vp9Downscaling
------------------------------------------------------------------------------*/
void Vp9DownscalingTest(vp9Instance_s *inst)
{
  u32 frame = (u32)inst->frameCnt;
  ...
  printf("Vp9DownscalingTest# %dx%d => %dx%d\n",
