...
------------------------------------------------------------------------------*/
static void Segmentation(vp9buffer *buffer, sps *sps, ppss *ppss,
                         entropy *entropy, mbs *mbs);
static void FilterLevelDelta(vp9buffer *buffer, sps *sps);
static void QuantDelta(vp9buffer *buffer, sps *sps);

/*------------------------------------------------------------------------------
  FrameHeader
------------------------------------------------------------------------------*/
void VP9FrameHeader(vp9Instance_s *container)
{
  ...
  vp9buffer *buffer = &container->buffer[1];  /* "Frame header" buffer */

  /* Color space and pixel Type (Key frames only) */
  if (cur_pic->i_frame)
  {
    VP9PutLit(buffer, sps->colorType, 1);
    //COMMENT("Frame header, buffer 1, color space type");

    VP9PutLit(buffer, sps->clampType, 1);
    //COMMENT("Frame header, buffer 1, clamping type");
  }

  /* Segmentation */
  VP9PutLit(buffer, pps->segmentEnabled, 1);
  ...
  VP9PutLit(buffer, sps->dctPartitions, 2);
  //COMMENT("Frame header, buffer 1, token partition");

  VP9PutLit(buffer, container->rateControl.qpHdr, 7);
  ...
  if (!cur_pic->i_frame)
  {
    ...
    VP9PutLit(buffer, cur_pic->grf, 1); /* Grf refresh */
    //COMMENT("Frame header, buffer 1, grf refresh");
    VP9PutLit(buffer, cur_pic->arf, 1); /* Arf refresh */
    ...
    VP9PutLit(buffer, sps->singBias[1], 1); /* Grf */
    //COMMENT("Frame header, buffer 1, grf sign bias");
    VP9PutLit(buffer, sps->singBias[2], 1); /* Arf */
...
/*------------------------------------------------------------------------------
  FrameTag
------------------------------------------------------------------------------*/
void VP9FrameTag(vp9Instance_s *container)
{
  picBuffer *picBuffer = &container->picBuffer;
  refPic *cur_pic = picBuffer->cur_pic;
  sps *sps = &container->sps;                 /* Sequence container */
  vp9buffer *buffer = &container->buffer[0];  /* Frame tag buffer */
  i32 tmp;
   ...
   * VP9PutLit() use VP9PutBit() instead */

  VP9PutByte(buffer, tmp & 0xff);
  //COMMENT("Frame tag, buffer 0, The first byte");

  VP9PutByte(buffer, (tmp >> 8) & 0xff);
  //COMMENT("Frame tag, buffer 0, The second byte");

  VP9PutByte(buffer, (tmp >> 16) & 0xff);
...
------------------------------------------------------------------------------*/
void VP9DataPartitionSizes(vp9Instance_s *container)
{
  sps *sps = &container->sps;   /* Sequence container */
  vp9buffer *buffer = container->buffer;  /* Data buffers */
  i32 i, tmp;
  ...
  for (i = 2; i < sps->partitionCnt - 1; i++)
  {
    ...
    VP9PutByte(&buffer[1], tmp & 0xff);
    //COMMENT("Partition size LSB");
    VP9PutByte(&buffer[1], (tmp >> 8) & 0xff);
    //COMMENT("Partition size");
    VP9PutByte(&buffer[1], (tmp >> 16) & 0xff);
...
/*------------------------------------------------------------------------------
  Segmentation
------------------------------------------------------------------------------*/
void Segmentation(vp9buffer *buffer, sps *sps, ppss *ppss, entropy *entropy,
                  mbs *mbs)
{
  pps *pps = ppss->pps;
  ...
  VP9PutLit(buffer, sgm->mapModified, 1);
  //COMMENT("Frame header, buffer 1, segmentation map modified");
  VP9PutLit(buffer, dataModified, 1);
  ...
  if (dataModified)
  {
    ...
    for (i = 0; i < SGM_CNT; i++)
    {
      ...
      VP9PutLit(buffer, ABS(tmp), 7);
      //COMMENT("Frame header, buffer 1, segment QP");
      VP9PutLit(buffer, tmp < 0, 1);
  ...
  if (sgm->mapModified)
  {
    ...
    for (i = 0; i < 3; i++)
    {
      if (sgm->idCnt[i] != 0)
      {
        ...
        VP9PutLit(buffer, entropy->segmentProb[i], 8);
...
/*------------------------------------------------------------------------------
    FilterLevelDelta
------------------------------------------------------------------------------*/
void FilterLevelDelta(vp9buffer *buffer, sps *sps)
{
  i32 i, tmp;
  i32 modeUpdate[4];
  i32 refUpdate[4];
  ...
  for (i = 0; i < 4; i++)
  {
    VP9PutLit(buffer, modeUpdate[i], 1);
