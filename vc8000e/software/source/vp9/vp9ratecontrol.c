...
static void PicSkip(vp9RateControl_s *rc, u32 layerId);
static void PicQuantLimit(vp9RateControl_s *rc);
static i32 VirtualBuffer(vp9VirtualBuffer_s *vb, i32 timeInc);
static void PicQuant(vp9RateControl_s *rc, u32 layerId);
static i32 avg_rc_error(linReg_s *p);
static void update_rc_error(linReg_s *p, i32 bits, i32 windowLen);
static i32 gop_avg_qp(vp9RateControl_s *rc);
...
/*------------------------------------------------------------------------------
  VP9InitRc() Initialize rate control.
------------------------------------------------------------------------------*/
void VP9InitRc(vp9RateControl_s *rc, u32 newStream)
{
  vp9VirtualBuffer_s *vb = rc->virtualBuffer;
  ...
  maxBps = VP9Calculate(maxBps, rc->outRateNum, rc->outRateDenom);
  if (maxBps < 0)
    maxBps = I32_MAX;
  if (totalBps > maxBps)
  {
    ...
    {
      vb[i].bitRate = VP9Calculate(vb[i].bitRate, maxBps, totalBps);
    }
  }

  for (i = 0; i < rc->layerAmount; i++)
  {
    vb[i].bitPerPic = VP9Calculate(vb[i].bitRate, vb[i].outRateDenom,
                                   rc->outRateNum);
  }
...
static i32 InitialQp(i32 bits, i32 pels)
{
  ...
  b = 290 - b * 120 / 8;
  bits /= b;
  bits = VP9Calculate(bits, upscale, pels);
...
static i32 VirtualBuffer(vp9VirtualBuffer_s *vb, i32 timeInc)
{
  ...
  vb->virtualBitCnt = VP9Calculate(vb->bitRate, vb->picTimeInc, vb->timeScale);
...
/*------------------------------------------------------------------------------
  VP9BeforePicRc()  Update virtual buffer and calculate picInitQp for current
  picture.
------------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------------
  VP9AfterPicRc()  Update RC statistics after encoding frame.
------------------------------------------------------------------------------*/
void VP9AfterPicRc(vp9RateControl_s *rc, u32 byteCnt, u32 layerId)
{
  vp9VirtualBuffer_s *vb = &rc->virtualBuffer[layerId];
  ...
  if (!rc->currFrameIntra)
  {
    tmp = VP9Calculate(bitCnt, DSCBITPERMB, rc->mbPerPic);
...
------------------------------------------------------------------------------*/
void PicSkip(vp9RateControl_s *rc, u32 layerId)
{
  vp9VirtualBuffer_s *vb = &rc->virtualBuffer[layerId];
...
------------------------------------------------------------------------------*/
void PicQuant(vp9RateControl_s *rc, u32 layerId)
{
  ...
  if (rc->currFrameIntra)
  {
    ...
    if (rc->prevFrameIntra)
    {
      ...
      normBits = VP9Calculate(targetBits, DSCBITPERMB, rc->mbPerPic);
...
/*------------------------------------------------------------------------------
  PicQuantLimit()
------------------------------------------------------------------------------*/
void PicQuantLimit(vp9RateControl_s *rc)
{
...
static i32 avg_rc_error(linReg_s *p)
{
  ...
  /* Avoid overflow */
  return VP9Calculate(p->bits[2], 4, 100) +
         VP9Calculate(p->bits[1], 2, 100);
}
...
------------------------------------------------------------------------------*/
i32 gop_avg_qp(vp9RateControl_s *rc)
{
  ...
  if (rc->gopBitCnt)
  {
    i32 gopIntraBitRatio = VP9Calculate(get_avg_bits(&rc->intra, 1),
