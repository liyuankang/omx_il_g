...
#define IS_VP9(a)   (a==VCENC_VIDEO_CODEC_VP9)
  ...
  typedef struct
  {
    ...
    u32 aq_mode; /* Mode for Adaptive Quantization - 0:none 1:uniform AQ 2:auto variance 3:auto variance with bias to dark scenes. Default 0 */
    double aq_strength; /* Reduces blocking and blurring in flat and textured areas (0 to 3.0). Default 1.00 */

    u32 writeReconToDDR; /*HW write recon to DDR or not if it's pure I-frame encoding*/

    u32 TxTypeSearchEnable; /*av1 tx type search 1=enable 0=disable, enabled by default*/
   } VCEncConfig;
  ...
  typedef struct
  {
	...
	ptr_t busAv1PrecarryOutBuf[MAX_STRM_BUF_NUM];     /* Bus address of AV1 precarry output buffer */
    ...
    VCEncPictureCodingType codingType;    /* Proposed picture coding type,
                                                 * INTRA/PREDICTED
                                                 */
    ptr_t busLumaStab;     /* bus address of next picture to stabilize (luma) */
...
#define LONG_TERM_REFERENCE	0x02
  ...
  typedef struct
  {
    ...
    u32 nextSceneChanged;    /* Scene change detection result for next frame. Filled by stabilization module. */
  ...
  typedef struct
  {
    ...
    u32 videoStabilization;                 /* Enable video stabilization: 0=disabled, 1=enabled. */
