...
void hantroenc_trigger_irq_0(unsigned long value)
{
  PDEBUG("trigger core 0 irq\n");
  del_timer(&timer0); 
  hantroenc_isr(0,(void *)&hantroenc_data[0]);
}
void hantroenc_trigger_irq_1(unsigned long value)
{
...
{   
    ...
    printk(KERN_INFO "Base hw val %p\n", (void *)gBaseHdwr);
    ...
    for (i=0; i<total_subsys_num; i++)
    {
      subsys_array[i].base_addr  = gBaseHdwr + subsys_array[i].base_addr;//the offset is based on which bus interface is chosen
    }
